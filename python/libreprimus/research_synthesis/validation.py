...
def validate_research_synthesis(
    ...
    if not staged_plan_path.is_file():
        ...
        _require_text(
            errors,
            staged_text,
            ("stage 5e", "first cuda kernel contract", "cpu/native parity adapter selection", "complete"),
            "staged_plan_stage5e_first_cuda_kernel_contract_complete",
        )
        _require_text(
            errors,
            staged_text,
        ...
        _require_text(
            errors,
            staged_text,
            ("stage 5i", "gematria mod-29", "synthetic cuda parity", "complete"),
            "staged_plan_stage5i_gematria_synthetic_cuda_parity_complete",
        )
        _require_text(
            errors,
            staged_text,
        ...
        _require_text(
            errors,
            staged_text,
            ("stage 5l", "gematria", "token mapping", "complete"),
            "staged_plan_stage5l_gematria_token_mapping_complete",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5m", "solved-fixture-safe", "cuda parity", "complete"),
            "staged_plan_stage5m_gematria_solved_fixture_cuda_parity_complete",
        )
        _require_text(
            errors,
            staged_text,
        ...
        _require_text(
            errors,
            staged_text,
            ("stage 5o", "solved-fixture-safe", "repeat verification", "result-store preflight", "complete"),
            "staged_plan_stage5o_gematria_repeat_result_store_preflight_complete",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5p", "controlled", "result-store"),
            "staged_plan_stage5p_controlled_result_store_complete",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5q", "controlled", "shift_score", "candidate mapping"),
            "staged_plan_stage5q_controlled_shift_score_complete",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5r", "controlled", "expanded", "shift_score", "complete"),
            "staged_plan_stage5r_controlled_expanded_shift_score_complete",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5s", "expanded solved-fixture", "result-store integration"),
            "staged_plan_stage5s_expanded_reporting_present",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5s", "expanded solved-fixture", "result-store integration", "complete"),
            "staged_plan_stage5s_expanded_reporting_complete",
        )
        _require_text(
            errors,
            staged_text,
        ...
        _require_text(
            ...
            "staged_plan_stage5u_present",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5u", "unified candidate batch abi", "backend contract", "complete"),
            "staged_plan_stage5u_candidate_batch_abi_complete",
        )
        _require_text(
            errors,
            staged_text,
        ...
        _require_text(
            errors,
            staged_text,
            ("stage 5x", "prime-minus-one stream", "no-gpu native parity", "complete"),
            "staged_plan_stage5x_prime_stream_native_parity_complete",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5y", "prime-minus-one native parity reporting"),
            "staged_plan_stage5y_next",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5y", "prime-minus-one native parity reporting", "complete"),
            "staged_plan_stage5y_prime_native_reporting_complete",
        )
        _require_text(
            errors,
            staged_text,
        ...
        _require_text(
            errors,
            staged_text,
            ("stage 5ab", "markdown staleness detection", "complete"),
            "staged_plan_stage5ab_next",
        )
        _require_text(
            errors,
            staged_text,
            ("stage 5ac", "selected from stage 5aa outcome", "stale-doc repair"),
            "staged_plan_stage5ac_next",
        )
    ...
    cuda_kernel_contract = _find_method(method_records, "cuda_first_kernel_contract")
    if cuda_kernel_contract is None:
        errors.append("cuda_first_kernel_contract_missing")
    else:
    ...
    if cuda_synthetic_shift is None:
        ...
        if (
            ...
            or "stage 5o" not in evidence_text
            or "stage 5p" not in evidence_text
    ...
    if native_abi_conformance is None:
        ...
        evidence_text = str(native_abi_conformance.get("evidence_summary", "")).lower()
        ...
        if (
            "stage 5w" not in evidence_text
            or "stage 5x" not in evidence_text
            or "stage 5y" not in evidence_text
            or "stage 5z" not in next_text
            or "prime-minus-one" not in next_text
        ):
    ...
    if prime_native_contract is None:
        ...
        if "stage 5x" not in evidence_text or "stage 5y" not in evidence_text or "stage 5z" not in next_text:
    ...
    if prime_native_parity is None:
        errors.append("prime_minus_one_native_parity_missing")
    else:
        stop_text = " ".join(str(item).lower() for item in prime_native_parity.get("stop_conditions", []))
        evidence_text = str(prime_native_parity.get("evidence_summary", "")).lower()
        next_text = str(prime_native_parity.get("next_action", "")).lower()
        if (
            "cuda" not in stop_text
            or "benchmark" not in stop_text
            or "full p56" not in stop_text
        ...
        if "stage 5y" not in evidence_text or "reporting" not in evidence_text or "stage 5z" not in next_text:
    ...
    if prime_native_reporting is None:
        errors.append("prime_minus_one_native_reporting_missing")
    else:
        stop_text = " ".join(str(item).lower() for item in prime_native_reporting.get("stop_conditions", []))
        evidence_text = str(prime_native_reporting.get("evidence_summary", "")).lower()
        next_text = str(prime_native_reporting.get("next_action", "")).lower()
        if (
