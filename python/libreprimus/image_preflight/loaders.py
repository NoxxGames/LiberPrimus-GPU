"""Load and write Stage 4M image preflight inputs and records."""
...
def load_jsonl_records(path: Path) -> list[dict[str, Any]]:
    """Load JSONL records from a committed metadata file."""
...
def write_yaml_document(path: Path, payload: dict[str, Any]) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        yaml.dump(payload, sort_keys=False, allow_unicode=False, Dumper=_NoAliasDumper),
        encoding="utf-8",
    )
