"""Optional local CUDA execution for Stage 5M solved-fixture parity."""
...
from libreprimus.gematria_solved_fixture_cuda.export import read_record_set, write_record_set, write_report, write_warnings
from libreprimus.gematria_solved_fixture_cuda.hash_contract import canonical_hash
from libreprimus.gematria_solved_fixture_cuda.models import BUILD_DIR, OUTPUT_DIR, OUTPUT_ORDERING, RUN_RECORDS_PATH, RUN_REPORT, TOKEN_DOMAIN
...
def run_cuda_parity(
    *,
    ...
    if skip_run:
        ...
        write_record_set(run_records_out, updated)
        write_report(out_dir, RUN_REPORT, {"records": updated})
    ...
    if build_status != "passed":
        ...
        write_warnings(out_dir, warnings)
        if require_cuda:
            raise RuntimeError(f"Stage 5M CUDA run required but build status is {build_status}: {build_reason}")
        return updated
    ...
    if not executable.is_file():
        ...
        write_record_set(run_records_out, updated)
        write_report(out_dir, RUN_REPORT, {"records": updated})
        write_warnings(out_dir, [{"level": "warning", "message": f"missing runner executable: {executable}"}])
        if require_cuda:
            raise RuntimeError(f"Stage 5M runner executable missing: {executable}")
        return updated
    ...
    if require_cuda and any(record["cuda_run_status"] != "passed" for record in updated):
        raise RuntimeError("Stage 5M required CUDA parity but at least one fixture did not pass")
    return updated
...
def _write_input(*, record: dict[str, Any], input_dir: Path) -> Path:
    mapping_id = str(record["mapping_id"]).replace("/", "_")
    path = input_dir / f"{mapping_id}.txt"
...
def _record_from_completed(*, record: dict[str, Any], completed: subprocess.CompletedProcess[str]) -> dict[str, Any]:
    ...
    updated["stage5n_ready"] = match
    return updated


def _cuda_hash(*, record: dict[str, Any], output_values: list[int]) -> str:
    token_count = len(record["token_records"])
    candidate_outputs = []
