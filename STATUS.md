...
Stage 5E first CUDA kernel contract and CPU/native parity adapter selection is complete. Next planned stage: Stage 5F first synthetic-only CUDA parity kernel implementation. Website expansion is deferred to Stage 6.
...
Stage 5F - first synthetic-only CUDA parity kernel implementation. Implement only the selected Stage 5E `shift_score_kernel` contract, keep it synthetic-only, and do not run GPU benchmarks, broad searches, raw-data processing, website expansion, or solve claims.
