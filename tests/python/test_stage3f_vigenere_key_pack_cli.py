...
def test_stage3f_vigenere_key_pack_cli_run_and_summary(tmp_path: Path) -> None:
    queue = load_bounded_queue(QUEUE).payload
    item = deepcopy(queue["items"][0])
    item["corpus_slice"]["slice_id"] = "stage3f-cli-synthetic-slice"
    item["corpus_slice"]["selector"] = {
        "selector_kind": "inline_index29_values",
        "page_candidate_id": "stage3f-cli-synthetic",
    ...
    assert "expected_candidate_count=48" in run.output
    assert "executed_candidate_count=48" in run.output
    assert "deferred_candidate_count=0" in run.output
    assert "vigenere_candidate_count=48" in run.output
