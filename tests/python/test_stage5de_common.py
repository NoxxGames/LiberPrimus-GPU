...
from libreprimus.token_block.stage5de import DATA_PATHS, SCHEMA_PATHS, build_stage5de

ROOT = Path(__file__).resolve().parents[2]
STAGE5DE_RECORDS = [path.as_posix() for path in DATA_PATHS.values()]
STAGE5DE_SCHEMAS = list(SCHEMA_PATHS.values())
SELECTED_OPTION_ID = "prepare_real_operator_approval_record"


def ensure_stage5de_built() -> None:
    build_stage5de()
