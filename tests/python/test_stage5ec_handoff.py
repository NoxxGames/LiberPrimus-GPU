...
def test_stage5ec_handoff_uses_codex_output_root_only() -> None:
    ensure_stage5ec_built()
    handoff = load_yaml("data/source-harvester/stage5ec-codex-handoff-policy.yaml")

    assert validate_stage5ec_handoff_continuity().validation_error_count == 0
...
def test_stage5ec_credential_redaction_policy_is_preserved() -> None:
    ensure_stage5ec_built()
    policy = load_yaml("data/source-harvester/stage5ec-credential-redaction-policy-preservation.yaml")

    assert validate_stage5ec_credential_redaction_policy().validation_error_count == 0
