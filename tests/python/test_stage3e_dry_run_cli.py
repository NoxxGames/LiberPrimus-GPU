...
def test_stage3e_dry_run_cli_writes_summary(tmp_path: Path) -> None:
    ...
    assert "item_count=6" in result.output
    assert "total_candidate_estimate=780" in result.output
    assert "runnable_now_count=1" in result.output
    assert "needs_executor_count=3" in result.output
    assert "dry_run_only_count=2" in result.output
