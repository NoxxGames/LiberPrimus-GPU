...
def test_operational_docs_use_stage5ce_latest_and_stage5cf_next() -> None:
    status = Path("STATUS.md").read_text(encoding="utf-8")
    staged_plan = Path("docs/roadmap/staged-plan.md").read_text(encoding="utf-8")
    assert "Stage 5CE active-planning-input proposal package" in status
    assert "Next recommended prompt: Stage 5CF" in status
    assert "Latest completed stage: Stage 5CE" in staged_plan
    assert "Current planning focus: Stage 5CF" in staged_plan
