...
def test_operational_docs_use_stage5ag_latest_and_stage5ah_next() -> None:
    status = Path("STATUS.md").read_text(encoding="utf-8")
    staged_plan = Path("docs/roadmap/staged-plan.md").read_text(encoding="utf-8")
    assert "Stage 5AG local third-party source inventory" in status
    assert "Next recommended prompt: Stage 5AH" in status
    assert "Latest completed stage: Stage 5AG" in staged_plan
    assert "Current planning focus: Stage 5AH" in staged_plan
