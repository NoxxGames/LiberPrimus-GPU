...
def test_operational_docs_use_stage5cm_latest_and_stage5cn_next() -> None:
    status = Path("STATUS.md").read_text(encoding="utf-8")
    staged_plan = Path("docs/roadmap/staged-plan.md").read_text(encoding="utf-8")
    assert "Stage 5CM approval-record readiness boundary" in status
    assert "Next recommended prompt: Stage 5CN" in status
    assert "Latest completed stage: Stage 5CM" in staged_plan
    assert "Current planning focus: Stage 5CN" in staged_plan
