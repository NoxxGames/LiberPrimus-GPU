...
def test_stage5ab_source_of_truth_schema_and_loader() -> None:
    ...
    assert source.latest_completed_stage_prefix == "Stage 5CS"
    assert source.expected_next_stage_prefix == "Stage 5CT"
    assert source.next_stage_after_this_stage == (
        "Stage 5CT - Deep Research review of Stage 5CS operator-decision "
        "readiness package and real-approval decision options scaffold, without execution"
    )
