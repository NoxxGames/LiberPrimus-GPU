...
def test_stage5ab_source_of_truth_schema_and_loader() -> None:
    ...
    assert source.latest_completed_stage_prefix == "Stage 5DR"
    assert source.expected_next_stage_prefix == "Stage 5DS"
    assert source.next_stage_after_this_stage == (
        "Stage 5DS - Operator Console source-review readiness planning, without puzzle execution"
    )
