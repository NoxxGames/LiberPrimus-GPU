...
def test_stage5ab_source_of_truth_schema_and_loader() -> None:
    ...
    assert source.latest_completed_stage_prefix == "Stage 5DT"
    assert source.expected_next_stage_prefix == "Stage 5DU"
    assert source.next_stage_after_this_stage == (
        "Stage 5DU - Operator/assistant source-lock number-fact review batch 1, without execution"
    )
