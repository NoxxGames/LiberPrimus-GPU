...
def test_stage5ab_source_of_truth_schema_and_loader() -> None:
    ...
    assert source.latest_completed_stage_prefix == "Stage 5DY"
    assert source.expected_next_stage_prefix == "Stage 5DZ"
    assert source.next_stage_after_this_stage == (
        "Stage 5DZ - Operator/assistant source-lock number-fact review batch 3, without execution"
    )
