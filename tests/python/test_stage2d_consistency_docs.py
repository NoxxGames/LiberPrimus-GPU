...
def _write_docs(tmp_path: Path, *, readme_extra: str = "", catalog_extra: str = "") -> dict[str, Path]:
    ...
    readme.write_text(
        ...
        "Stage 2F complete. Stage 2G complete. Next Stage 2H.\n"
        + ("Line\n" * 60)
        + readme_extra,
        encoding="utf-8",
    )
    status.write_text(
        "# Status\n\nStage 2D current. Stage 2E complete. Stage 2F complete. Stage 2G complete.\n"
        + ("Line\n" * 30),
        encoding="utf-8",
    )
    roadmap.write_text(
        "# Roadmap\n\nStage 2D next. Stage 2E complete. Stage 2F complete. Stage 2G complete.\n"
        + ("Line\n" * 30),
        encoding="utf-8",
    )
    agents.write_text(
