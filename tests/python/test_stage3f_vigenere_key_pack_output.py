...
def _synthetic_item() -> dict:
    ...
    item["corpus_slice"]["slice_id"] = "stage3f-output-synthetic-slice"
    item["corpus_slice"]["selector"] = {
        "selector_kind": "inline_index29_values",
        "page_candidate_id": "stage3f-output-synthetic",
...
def test_stage3f_candidate_output_records_include_key_and_modes(tmp_path: Path) -> None:
...
def test_stage3f_run_summary_records_expected_executed_and_deferred_counts(tmp_path: Path) -> None:
    summary = run_vigenere_key_pack_item(_synthetic_item(), out_dir=tmp_path, top_k=5)
    payload = json.loads(Path(summary.output_paths["summary"]).read_text(encoding="utf-8"))

    assert payload["expected_candidate_count"] == 48
    assert payload["executed_candidate_count"] == 48
...
def test_stage3f_generated_outputs_are_ignored() -> None:
    path = "experiments/results/bounded-auto-runs/stage3f/candidate_records.jsonl"
