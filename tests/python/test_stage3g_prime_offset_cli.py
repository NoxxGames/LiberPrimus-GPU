...
def test_stage3g_prime_offset_cli_run_and_summary(tmp_path: Path) -> None:
    ...
    item["corpus_slice"]["slice_id"] = "stage3g-cli-synthetic-slice"
    item["corpus_slice"]["selector"] = {
        "selector_kind": "inline_index29_values",
        "page_candidate_id": "stage3g-cli-synthetic",
    ...
    assert "expected_candidate_count=256" in run.output
    assert "executed_candidate_count=256" in run.output
    assert "deferred_candidate_count=0" in run.output
    assert "prime_candidate_count=256" in run.output
    assert "solve_claim=false" in run.output
    assert summary.exit_code == 0, summary.output
    assert "top_candidate_offset=" in summary.output
    assert "top_candidate_direction=" in summary.output
