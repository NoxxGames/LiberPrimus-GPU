...
try {
    ...
    Write-Host "Validating Stage 5EH Lag5/outguess/byte-control source-lock addendum records"
    & $Python -m libreprimus.cli token-block validate-stage5eh
...
"@ | Set-Content -Encoding UTF8 $Stage5DFakeNative
    ...
    Write-Host "Validating Stage 5AN private Deep Research export records"
    $Stage5ANRoot = Join-Path $TempDir "stage5an-deep-research-export"
    $Stage5ANPack = Join-Path $Stage5ANRoot "content-pack"
    $Stage5ANHosted = Join-Path $Stage5ANRoot "private-content"
    $Stage5ANWebroot = Join-Path $Stage5ANRoot "webserver-root"
    $Stage5ANRecords = Join-Path $Stage5ANRoot "records"
    New-Item -ItemType Directory -Path $Stage5ANRoot, $Stage5ANRecords | Out-Null
    $Stage5ANPolicy = Join-Path $Stage5ANRecords "content-pack-policy.yaml"
    $Stage5ANInputs = Join-Path $Stage5ANRecords "content-pack-inputs.yaml"
    $Stage5ANManifestSummary = Join-Path $Stage5ANRecords "content-pack-manifest-summary.yaml"
    $Stage5ANHostedSummary = Join-Path $Stage5ANRecords "hosted-content-export-summary.yaml"
    $Stage5ANCombinedSummary = Join-Path $Stage5ANRecords "combined-webroot-summary.yaml"
    $Stage5ANFileSelection = Join-Path $Stage5ANRecords "file-selection-summary.yaml"
    $Stage5ANPublicationGateAudit = Join-Path $Stage5ANRecords "publication-gate-audit.yaml"
    $Stage5ANUploadInstructions = Join-Path $Stage5ANRecords "upload-instructions.yaml"
    $Stage5ANConsumptionGuide = Join-Path $Stage5ANRecords "deep-research-consumption-guide.yaml"
    $Stage5ANGuardrail = Join-Path $Stage5ANRecords "guardrail.yaml"
    $Stage5ANNextStageDecision = Join-Path $Stage5ANRecords "next-stage-decision.yaml"
    $Stage5ANSummary = Join-Path $Stage5ANRecords "summary.yaml"
    & $Python -m libreprimus.cli deep-research-export build-stage5an-content-pack `
        --metadata-site-root $Stage5AMSite `
        --website-ingest-dir data/website-ingest/stage5al `
        ...
        --out-root $Stage5ANPack `
        --policy-out $Stage5ANPolicy `
        --inputs-out $Stage5ANInputs `
        --manifest-summary-out $Stage5ANManifestSummary `
        --file-selection-summary-out $Stage5ANFileSelection `
        --publication-gate-audit-out $Stage5ANPublicationGateAudit
    & $Python -m libreprimus.cli deep-research-export build-stage5an-hosted-export `
        --content-pack-root $Stage5ANPack `
        --metadata-site-root $Stage5AMSite `
        --out-root $Stage5ANHosted `
        --summary-out $Stage5ANHostedSummary `
        --upload-instructions-out $Stage5ANUploadInstructions `
        --consumption-guide-out $Stage5ANConsumptionGuide
    & $Python -m libreprimus.cli deep-research-export build-stage5an-combined-webroot `
        --metadata-site-root $Stage5AMSite `
        --private-content-root $Stage5ANHosted `
        --out-root $Stage5ANWebroot `
        --summary-out $Stage5ANCombinedSummary
    & $Python -m libreprimus.cli deep-research-export build-stage5an-guardrail `
        --content-pack-root $Stage5ANPack `
        --hosted-export-root $Stage5ANHosted `
        --combined-webroot $Stage5ANWebroot `
        --out $Stage5ANGuardrail
    & $Python -m libreprimus.cli deep-research-export build-stage5an-next-stage-decision `
        --manifest-summary $Stage5ANManifestSummary `
        --hosted-summary $Stage5ANHostedSummary `
        --combined-summary $Stage5ANCombinedSummary `
        --publication-gate-audit $Stage5ANPublicationGateAudit `
        --out $Stage5ANNextStageDecision
    & $Python -m libreprimus.cli deep-research-export build-stage5an-summary `
        --policy $Stage5ANPolicy `
        --inputs $Stage5ANInputs `
        --manifest-summary $Stage5ANManifestSummary `
        --hosted-summary $Stage5ANHostedSummary `
        --combined-summary $Stage5ANCombinedSummary `
        --file-selection-summary $Stage5ANFileSelection `
        --publication-gate-audit $Stage5ANPublicationGateAudit `
        --upload-instructions $Stage5ANUploadInstructions `
        --consumption-guide $Stage5ANConsumptionGuide `
        --guardrail $Stage5ANGuardrail `
        --next-stage-decision $Stage5ANNextStageDecision `
        --out $Stage5ANSummary
    & $Python -m libreprimus.cli deep-research-export validate-stage5an `
        --content-pack-root $Stage5ANPack `
        --hosted-export-root $Stage5ANHosted `
        --combined-webroot $Stage5ANWebroot `
        --policy $Stage5ANPolicy `
    ...
    Write-Host "Validating Stage 5AR original-image coordinate-lock records"
    $Stage5ARResultsRoot = Join-Path (Join-Path "experiments" "results") "token-block/stage5ar"
    & $Python -m libreprimus.cli token-block validate-stage5ar `
        ...
        --results-dir $Stage5ARResultsRoot
    git check-ignore -q (Join-Path $Stage5ARResultsRoot "token_pixel_coordinate_records.jsonl")
    git check-ignore -q (Join-Path $Stage5ARResultsRoot "token_coordinate_validation.json")
    git check-ignore -q (Join-Path $Stage5ARResultsRoot "review-overlays/example.png")
    git check-ignore -q "codex-output/stage5ar-codex-completion.md"

    Write-Host "Validating Stage 5AT token case-review pack records"
    $Stage5ATResultsRoot = Join-Path (Join-Path "experiments" "results") "token-block/stage5at"
    $Stage5ATReviewPackRoot = Join-Path (Join-Path "human-review-packs" "stage5at") "token-case-review"
    & $Python -m libreprimus.cli token-block validate-stage5at `
        ...
        --review-pack-root $Stage5ATReviewPackRoot `
        --results-dir $Stage5ATResultsRoot
    git check-ignore -q (Join-Path $Stage5ATResultsRoot "case_review_challenge_set.json")
    ...
    Write-Host "Validating Stage 5AW decision parser repair records"
    $Stage5AWResultsRoot = Join-Path (Join-Path "experiments" "results") "token-block/stage5aw"
    & $Python -m libreprimus.cli token-block validate-stage5aw `
        ...
        --results-dir $Stage5AWResultsRoot
...
"@ | & $Python -
    ...
    Write-Host "Validating Stage 5AY bounded token-block preflight design records"
    $Stage5AYResultsRoot = Join-Path (Join-Path "experiments" "results") "token-block/stage5ay"
    & $Python -m libreprimus.cli token-block validate-stage5ay `
        ...
        --results-dir $Stage5AYResultsRoot
    ...
    Write-Host "Validating Stage 5BF historical route source-lock records"
    $Stage5BFResultsRoot = Join-Path (Join-Path "experiments" "results") "historical-route/stage5bf"
    & $Python -m libreprimus.cli historical-route validate-stage5bf --results-dir $Stage5BFResultsRoot
    git check-ignore -q (Join-Path $Stage5BFResultsRoot "full_archive_file_inventory.jsonl")
    git check-ignore -q (Join-Path $Stage5BFResultsRoot "high_priority_artifact_index.json")
    git check-ignore -q (Join-Path $Stage5BFResultsRoot "summary.json")
    ...
    Write-Host "Validating Stage 5BM String 4 branch-crosswalk repair records"
    & $Python -m libreprimus.cli token-block validate-stage5bm
    $Stage5BMTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5bm"
    $Stage5BMHistoricalResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "historical-route") "stage5bm"
    git check-ignore -q (Join-Path $Stage5BMTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BMTokenResultsRoot "string4-stage5ap-mismatch-table.json")
    git check-ignore -q (Join-Path $Stage5BMHistoricalResultsRoot "summary.json")
    git check-ignore -q "codex-output/stage5bm-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5BM" }

    Write-Host "Validating Stage 5BN String 4 unsupported-position source-gap records"
    & $Python -m libreprimus.cli token-block validate-stage5bn
    $Stage5BNTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5bn"
    $Stage5BNHistoricalResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "historical-route") "stage5bn"
    git check-ignore -q (Join-Path $Stage5BNTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BNTokenResultsRoot "target-position-audit.json")
    git check-ignore -q (Join-Path $Stage5BNHistoricalResultsRoot "summary.json")
    ...
    Write-Host "Validating Stage 5BQ inactive-branch dry-run planning integration records"
    & $Python -m libreprimus.cli token-block validate-stage5bq
    $Stage5BQTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5bq"
    git check-ignore -q (Join-Path $Stage5BQTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BQTokenResultsRoot "source_file_digests.json")
    git check-ignore -q (Join-Path $Stage5BQTokenResultsRoot "warnings.jsonl")
    git check-ignore -q "codex-output/stage5bq-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5BQ" }

    Write-Host "Validating Stage 5BS planning-ingestion gate records"
    & $Python -m libreprimus.cli token-block validate-stage5bs
    $Stage5BSTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5bs"
    git check-ignore -q (Join-Path $Stage5BSTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BSTokenResultsRoot "source_file_digests.json")
    git check-ignore -q (Join-Path $Stage5BSTokenResultsRoot "warnings.jsonl")
    git check-ignore -q "codex-output/stage5bs-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5BS" }

    Write-Host "Validating Stage 5BU lineage-path reviewability hardening records"
    & $Python -m libreprimus.cli token-block validate-stage5bu-lineage-paths
    & $Python -m libreprimus.cli token-block validate-stage5bu
    $Stage5BUTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5bu"
    git check-ignore -q (Join-Path $Stage5BUTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BUTokenResultsRoot "source_file_digests.json")
    git check-ignore -q (Join-Path $Stage5BUTokenResultsRoot "lineage_path_resolution.json")
    git check-ignore -q (Join-Path $Stage5BUTokenResultsRoot "warnings.jsonl")
    git check-ignore -q "codex-output/stage5bu-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5BU" }

    Write-Host "Validating Stage 5BW inactive-sidecar planning-ingestion preflight records"
    & $Python -m libreprimus.cli token-block validate-stage5bw
    $Stage5BWTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5bw"
    git check-ignore -q (Join-Path $Stage5BWTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BWTokenResultsRoot "sidecar-ingestion-preflight.json")
    git check-ignore -q (Join-Path $Stage5BWTokenResultsRoot "manifest-supersession-preflight.json")
    git check-ignore -q (Join-Path $Stage5BWTokenResultsRoot "no-active-ingestion-proof.json")
    git check-ignore -q "codex-output/stage5bw-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5BW" }

    Write-Host "Validating Stage 5BY inactive-sidecar planning-manifest scaffold records"
    ...
    & $Python -m libreprimus.cli token-block validate-stage5by
    $Stage5BYTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5by"
    git check-ignore -q (Join-Path $Stage5BYTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5BYTokenResultsRoot "sidecar-planning-manifest-scaffold.json")
    git check-ignore -q (Join-Path $Stage5BYTokenResultsRoot "no-execution-planning-ingestion-sidecar.json")
    git check-ignore -q (Join-Path $Stage5BYTokenResultsRoot "source-digest-duplicate-review.json")
    git check-ignore -q "codex-output/stage5by-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5BY" }

    Write-Host "Validating Stage 5CA inactive-sidecar review-contract hardening records"
    ...
    & $Python -m libreprimus.cli token-block validate-stage5ca-manifest-supersession-contract
    ...
    $Stage5CATokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5ca"
    git check-ignore -q (Join-Path $Stage5CATokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CATokenResultsRoot "citation_contract.json")
    git check-ignore -q (Join-Path $Stage5CATokenResultsRoot "activation_precondition_contract.json")
    git check-ignore -q (Join-Path $Stage5CATokenResultsRoot "manifest_supersession_preflight_contract.json")
    git check-ignore -q "codex-output/stage5ca-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CA" }

    Write-Host "Validating Stage 5CC active-planning-input preflight records"
    ...
    & $Python -m libreprimus.cli token-block validate-stage5cc-active-planning-input-preflight
    & $Python -m libreprimus.cli token-block validate-stage5cc-no-byte-stream-transition-gate
    & $Python -m libreprimus.cli token-block validate-stage5cc-no-execution-transition-gate
    ...
    $Stage5CCTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cc"
    git check-ignore -q (Join-Path $Stage5CCTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CCTokenResultsRoot "fail_closed_trigger_contract.json")
    git check-ignore -q (Join-Path $Stage5CCTokenResultsRoot "activation_precondition_contract.json")
    git check-ignore -q (Join-Path $Stage5CCTokenResultsRoot "no_byte_stream_transition_gate.json")
    git check-ignore -q "codex-output/stage5cc-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CC" }

    Write-Host "Validating Stage 5CE active-planning-input proposal package records"
    & $Python -m libreprimus.cli token-block validate-stage5ce-proposal-package
    & $Python -m libreprimus.cli token-block validate-stage5ce-approval-gate
    & $Python -m libreprimus.cli token-block validate-stage5ce-citation-negative-tests
    & $Python -m libreprimus.cli token-block validate-stage5ce-no-byte-stream-transition-gate
    & $Python -m libreprimus.cli token-block validate-stage5ce-no-execution-transition-gate
    & $Python -m libreprimus.cli token-block validate-stage5ce
    $Stage5CETokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5ce"
    git check-ignore -q (Join-Path $Stage5CETokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CETokenResultsRoot "proposal_package.json")
    git check-ignore -q (Join-Path $Stage5CETokenResultsRoot "combined_gate_contract.json")
    git check-ignore -q "codex-output/stage5ce-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CE" }

    Write-Host "Validating Stage 5CG approval-gate decision scaffold records"
    & $Python -m libreprimus.cli token-block validate-stage5cg-operator-decision-scaffold
    & $Python -m libreprimus.cli token-block validate-stage5cg-deep-research-decision-scaffold
    & $Python -m libreprimus.cli token-block validate-stage5cg-combined-approval-gate
    & $Python -m libreprimus.cli token-block validate-stage5cg-active-planning-input-decision-scaffold
    & $Python -m libreprimus.cli token-block validate-stage5cg-stage5ce-wording-review
    & $Python -m libreprimus.cli token-block validate-stage5cg-no-byte-stream-transition-gate
    & $Python -m libreprimus.cli token-block validate-stage5cg-no-execution-transition-gate
    & $Python -m libreprimus.cli token-block validate-stage5cg-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5cg
    $Stage5CGTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cg"
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "wording_review.json")
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "decision_scaffold_report.json")
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "approval_decision_scaffolds.json")
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "proposal_package_preservation.json")
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "no_byte_stream_transition_gate.json")
    git check-ignore -q (Join-Path $Stage5CGTokenResultsRoot "no_execution_transition_gate.json")
    git check-ignore -q "codex-output/stage5cg-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CG" }

    Write-Host "Validating Stage 5CI approval-record template hardening records"
    ...
    $Stage5CITokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5ci"
    git check-ignore -q (Join-Path $Stage5CITokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CITokenResultsRoot "approval_template_report.json")
    git check-ignore -q (Join-Path $Stage5CITokenResultsRoot "activation_decision_template.json")
    git check-ignore -q (Join-Path $Stage5CITokenResultsRoot "combined_gate_validation.json")
    git check-ignore -q (Join-Path $Stage5CITokenResultsRoot "negative_validation_contract.json")
    git check-ignore -q "codex-output/stage5ci-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CI" }

    Write-Host "Validating Stage 5CK approval-record fixture pack records"
    ...
    $Stage5CKTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5ck"
    git check-ignore -q (Join-Path $Stage5CKTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CKTokenResultsRoot "fixture_pack_report.json")
    git check-ignore -q (Join-Path $Stage5CKTokenResultsRoot "activation_decision_review_package.json")
    git check-ignore -q (Join-Path $Stage5CKTokenResultsRoot "combined_gate_validation.json")
    ...
    Write-Host "Validating Stage 5CO real approval-record readiness transition records"
    ...
    $Stage5COTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5co"
    git check-ignore -q (Join-Path $Stage5COTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5COTokenResultsRoot "readiness_package_report.json")
    git check-ignore -q (Join-Path $Stage5COTokenResultsRoot "transition_plan_report.json")
    git check-ignore -q (Join-Path $Stage5COTokenResultsRoot "credential_scan.json")
    git check-ignore -q (Join-Path $Stage5COTokenResultsRoot "source_digest_index.json")
    git check-ignore -q "codex-output/stage5co-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CO" }

    Write-Host "Validating Stage 5CQ operator-decision package scaffold records"
    ...
    $Stage5CQTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cq"
    git check-ignore -q (Join-Path $Stage5CQTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CQTokenResultsRoot "operator_decision_package_report.json")
    git check-ignore -q (Join-Path $Stage5CQTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5CQTokenResultsRoot "handoff_restoration_report.json")
    git check-ignore -q (Join-Path $Stage5CQTokenResultsRoot "source_digest_index.json")
    git check-ignore -q "codex-output/stage5cq-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CQ" }

    Write-Host "Validating Stage 5CS operator-decision readiness/options records"
    & $Python -m libreprimus.cli token-block build-stage5cs
    & $Python -m libreprimus.cli token-block validate-stage5cs-stage5cr-findings
    & $Python -m libreprimus.cli token-block validate-stage5cs-operator-decision-readiness
    & $Python -m libreprimus.cli token-block validate-stage5cs-decision-options
    & $Python -m libreprimus.cli token-block validate-stage5cs-options-nonselection
    & $Python -m libreprimus.cli token-block validate-stage5cs-real-record-blocker
    & $Python -m libreprimus.cli token-block validate-stage5cs-combined-gate
    & $Python -m libreprimus.cli token-block validate-stage5cs-activation-nonauthorization
    & $Python -m libreprimus.cli token-block validate-stage5cs-stage5cq-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cs-stage5co-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cs-prior-stage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cs-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5cs-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5cs-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5cs
    $Stage5CSTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cs"
    git check-ignore -q (Join-Path $Stage5CSTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CSTokenResultsRoot "operator_decision_readiness_report.json")
    git check-ignore -q (Join-Path $Stage5CSTokenResultsRoot "decision_options_report.json")
    git check-ignore -q (Join-Path $Stage5CSTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5CSTokenResultsRoot "handoff_continuity_report.json")
    git check-ignore -q (Join-Path $Stage5CSTokenResultsRoot "source_digest_index.json")
    git check-ignore -q "codex-output/stage5cs-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CS" }

    Write-Host "Validating Stage 5CU operator-decision option negative-fixture hardening records"
    ...
    $Stage5CUTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cu"
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "operator_decision_readiness_report.json")
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "decision_options_report.json")
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "negative_fixture_report.json")
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "handoff_continuity_report.json")
    git check-ignore -q (Join-Path $Stage5CUTokenResultsRoot "source_digest_index.json")
    git check-ignore -q "codex-output/stage5cu-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CU" }

    Write-Host "Validating Stage 5CW real-decision package preflight records"
    ...
    $Stage5CWTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cw"
    git check-ignore -q (Join-Path $Stage5CWTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CWTokenResultsRoot "real_decision_package_preflight_report.json")
    git check-ignore -q (Join-Path $Stage5CWTokenResultsRoot "preflight_misuse_report.json")
    git check-ignore -q (Join-Path $Stage5CWTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5CWTokenResultsRoot "handoff_continuity_report.json")
    git check-ignore -q (Join-Path $Stage5CWTokenResultsRoot "source_digest_index.json")
    git check-ignore -q "codex-output/stage5cw-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CW" }

    Write-Host "Validating Stage 5CY option-selection decision preflight records"
    & $Python -m libreprimus.cli token-block build-stage5cy
    & $Python -m libreprimus.cli token-block validate-stage5cy-stage5cx-findings
    & $Python -m libreprimus.cli token-block validate-stage5cy-real-decision-preflight-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cy-operator-option-selection-preflight
    & $Python -m libreprimus.cli token-block validate-stage5cy-option-selection-requirements
    & $Python -m libreprimus.cli token-block validate-stage5cy-option-selection-misuse
    & $Python -m libreprimus.cli token-block validate-stage5cy-options-nonselection
    & $Python -m libreprimus.cli token-block validate-stage5cy-validation-count-reconciliation
    & $Python -m libreprimus.cli token-block validate-stage5cy-real-record-blocker
    & $Python -m libreprimus.cli token-block validate-stage5cy-combined-gate
    & $Python -m libreprimus.cli token-block validate-stage5cy-activation-nonauthorization
    & $Python -m libreprimus.cli token-block validate-stage5cy-stage5cw-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cy-stage5cu-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cy-stage5cs-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cy-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cy-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5cy-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5cy-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5cy-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5cy-governance-scope-control
    & $Python -m libreprimus.cli token-block validate-stage5cy
    $Stage5CYTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5cy"
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "option_selection_preflight_report.json")
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "option_selection_misuse_report.json")
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "validation_count_reconciliation_report.json")
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "handoff_continuity_report.json")
    git check-ignore -q (Join-Path $Stage5CYTokenResultsRoot "source_digest_index.json")
    git check-ignore -q "codex-output/stage5cy-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5CY" }

    Write-Host "Validating Stage 5DA operator choice / pause scaffold records"
    & $Python -m libreprimus.cli token-block build-stage5da
    & $Python -m libreprimus.cli token-block validate-stage5da-stage5cz-findings
    & $Python -m libreprimus.cli token-block validate-stage5da-choice-pause-scaffold
    & $Python -m libreprimus.cli token-block validate-stage5da-choice-pause-nonselection
    & $Python -m libreprimus.cli token-block validate-stage5da-explicit-pause-nonactivation
    & $Python -m libreprimus.cli token-block validate-stage5da-real-record-blocker
    & $Python -m libreprimus.cli token-block validate-stage5da-stage5cy-preservation
    & $Python -m libreprimus.cli token-block validate-stage5da-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5da-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5da-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5da-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5da-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5da-governance-scope-control
    & $Python -m libreprimus.cli token-block validate-stage5da
    $Stage5DATokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5da"
    git check-ignore -q (Join-Path $Stage5DATokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5DATokenResultsRoot "choice_pause_scaffold_report.json")
    git check-ignore -q (Join-Path $Stage5DATokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5DATokenResultsRoot "handoff_continuity_report.json")
    ...
    Write-Host "Validating Stage 5DG real operator approval record creation records"
    & $Python -m libreprimus.cli token-block build-stage5dg
    & $Python -m libreprimus.cli token-block validate-stage5dg-stage5df-findings
    & $Python -m libreprimus.cli token-block validate-stage5dg-real-operator-approval-record
    & $Python -m libreprimus.cli token-block validate-stage5dg-operator-approval-scope
    & $Python -m libreprimus.cli token-block validate-stage5dg-operator-approval-nonactivation
    & $Python -m libreprimus.cli token-block validate-stage5dg-stage5de-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dg-stage5dc-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dg-selected-option-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dg-unselected-options-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dg-real-record-boundary
    & $Python -m libreprimus.cli token-block validate-stage5dg-deep-research-absence
    & $Python -m libreprimus.cli token-block validate-stage5dg-combined-gate
    & $Python -m libreprimus.cli token-block validate-stage5dg-activation-nonauthorization
    & $Python -m libreprimus.cli token-block validate-stage5dg-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dg-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dg-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5dg-target-context
    & $Python -m libreprimus.cli token-block validate-stage5dg-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5dg-governance-scope
    & $Python -m libreprimus.cli token-block validate-stage5dg-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5dg
    & $Python -m libreprimus.cli token-block stage5dg-summary
    $Stage5DGTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5dg"
    git check-ignore -q (Join-Path $Stage5DGTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5DGTokenResultsRoot "approval_record_report.json")
    git check-ignore -q (Join-Path $Stage5DGTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5DGTokenResultsRoot "boundary_report.json")
    git check-ignore -q (Join-Path $Stage5DGTokenResultsRoot "handoff_continuity_report.json")
    git check-ignore -q (Join-Path $Stage5DGTokenResultsRoot "warnings.jsonl")
    git check-ignore -q "codex-output/stage5dg-codex-completion.md"
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5DG" }

    Write-Host "Validating Stage 5DI recent clue source-lock and pivot-readiness records"
    & $Python -m libreprimus.cli token-block build-stage5di
    & $Python -m libreprimus.cli token-block validate-stage5di-source-lock-register
    & $Python -m libreprimus.cli token-block validate-stage5di-local-archive-crosswalk
    & $Python -m libreprimus.cli token-block validate-stage5di-number-triangle-crosswalk
    & $Python -m libreprimus.cli token-block validate-stage5di-route-candidate-families
    & $Python -m libreprimus.cli token-block validate-stage5di-pivot-readiness
    & $Python -m libreprimus.cli token-block validate-stage5di-dinkus-visual-delimiter
    & $Python -m libreprimus.cli token-block validate-stage5di-stage5dg-preservation
    & $Python -m libreprimus.cli token-block validate-stage5di-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5di-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5di-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5di-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5di-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5di-governance-scope
    & $Python -m libreprimus.cli token-block validate-stage5di
    & $Python -m libreprimus.cli token-block stage5di-summary
    $Stage5DITokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5di"
    git check-ignore -q (Join-Path $Stage5DITokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5DITokenResultsRoot "source_lock_report.json")
    git check-ignore -q (Join-Path $Stage5DITokenResultsRoot "pivot_readiness_report.json")
    git check-ignore -q (Join-Path $Stage5DITokenResultsRoot "candidate_family_report.json")
    git check-ignore -q (Join-Path $Stage5DITokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5DITokenResultsRoot "handoff_continuity_report.json")
    ...
    Write-Host "Validating Stage 5DL triangle / disk / quote / koan source-lock records"
    & $Python -m libreprimus.cli token-block validate-stage5dl-number-triangle-v1
    & $Python -m libreprimus.cli token-block validate-stage5dl-triangle-way-anchor
    & $Python -m libreprimus.cli token-block validate-stage5dl-triangle-prime-mask
    & $Python -m libreprimus.cli token-block validate-stage5dl-triangle-2016-prime-layer
    & $Python -m libreprimus.cli token-block validate-stage5dl-triangle-fibonacci-prime-index
    & $Python -m libreprimus.cli token-block validate-stage5dl-triangle-56311-wynn-way
    & $Python -m libreprimus.cli token-block validate-stage5dl-disk-cipher-source-lock
    & $Python -m libreprimus.cli token-block validate-stage5dl-quote-dialogue-cribs
    & $Python -m libreprimus.cli token-block validate-stage5dl-koan-depiction-source-lock
    & $Python -m libreprimus.cli token-block validate-stage5dl-local-source-crosswalks
    & $Python -m libreprimus.cli token-block validate-stage5dl-pivot-readiness
    & $Python -m libreprimus.cli token-block validate-stage5dl-stage5dg-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dl-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dl-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dl-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5dl-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5dl-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5dl-governance-scope
    & $Python -m libreprimus.cli token-block validate-stage5dl
    & $Python -m libreprimus.cli token-block stage5dl-summary
    $Stage5DLTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5dl"
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "source_lock_report.json")
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "candidate_family_report.json")
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "pivot_readiness_report.json")
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "handoff_continuity_report.json")
    git check-ignore -q (Join-Path $Stage5DLTokenResultsRoot "warnings.jsonl")
    ...
    Write-Host "Validating Stage 5DN DiskCipher v1 source-lock and bridge records"
    & $Python -m libreprimus.cli token-block validate-stage5dn-disk-source-lock
    & $Python -m libreprimus.cli token-block validate-stage5dn-results-png
    & $Python -m libreprimus.cli token-block validate-stage5dn-message-bodies
    & $Python -m libreprimus.cli token-block validate-stage5dn-disk-56311-wynn-way
    & $Python -m libreprimus.cli token-block validate-stage5dn-disk-p39-row1-cluster
    & $Python -m libreprimus.cli token-block validate-stage5dn-doublet-suppression
    & $Python -m libreprimus.cli token-block validate-stage5dn-probability-quarantine
    & $Python -m libreprimus.cli token-block validate-stage5dn-circumference-precedent
    & $Python -m libreprimus.cli token-block validate-stage5dn-pdd-triangle-56311-update
    & $Python -m libreprimus.cli token-block validate-stage5dn-stage5dm-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dn-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dn-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dn-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5dn-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5dn-governance-scope
    & $Python -m libreprimus.cli token-block validate-stage5dn
    & $Python -m libreprimus.cli token-block stage5dn-summary
    $Stage5DNTokenResultsRoot = Join-Path (Join-Path (Join-Path "experiments" "results") "token-block") "stage5dn"
    git check-ignore -q (Join-Path $Stage5DNTokenResultsRoot "summary.json")
    git check-ignore -q (Join-Path $Stage5DNTokenResultsRoot "disk_source_lock_report.json")
    git check-ignore -q (Join-Path $Stage5DNTokenResultsRoot "disk_file_inventory.json")
    git check-ignore -q (Join-Path $Stage5DNTokenResultsRoot "candidate_bridge_report.json")
    git check-ignore -q (Join-Path $Stage5DNTokenResultsRoot "preservation_report.json")
    git check-ignore -q (Join-Path $Stage5DNTokenResultsRoot "warnings.jsonl")
    ...
    Write-Host "Validating Stage 5DV Source Browser performance and path repair records"
    & $Python -m libreprimus.cli token-block validate-stage5dv
    & $Python -m libreprimus.cli token-block stage5dv-summary
    & $Python -m libreprimus.cli token-block validate-stage5dv-source-browser-performance
    & $Python -m libreprimus.cli token-block validate-stage5dv-path-canonicalization
    & $Python -m libreprimus.cli token-block validate-stage5dv-chatgpt-context
    & $Python -m libreprimus.cli token-block validate-stage5dv-source-browser-loadability
    & $Python -m libreprimus.cli token-block validate-stage5dv-stage5du-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dv-stage5dt-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dv-stage5dg-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dv-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dv-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dv-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5dv-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5dv-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5dv-governance-scope
    & $Python -m libreprimus.cli source-browser validate-paths
    & $Python -m libreprimus.cli source-browser performance-smoke
    ...
    if (Test-Path "codex_output") { throw "codex_output must not be used for Stage 5DV" }

    Write-Host "Validating Stage 5DW number-fact review batch overlay records"
    & $Python -m libreprimus.cli token-block validate-stage5dw
    & $Python -m libreprimus.cli token-block stage5dw-summary
    & $Python -m libreprimus.cli token-block validate-stage5dw-review-batch-selection
    & $Python -m libreprimus.cli token-block validate-stage5dw-number-fact-overlays
    & $Python -m libreprimus.cli token-block validate-stage5dw-overlay-only-fact-cards
    & $Python -m libreprimus.cli token-block validate-stage5dw-source-browser-loadability
    & $Python -m libreprimus.cli token-block validate-stage5dw-stage5dv-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dw-stage5dg-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dw-stage5bd-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dw-active-lineage-preservation
    & $Python -m libreprimus.cli token-block validate-stage5dw-sidecar-gates
    & $Python -m libreprimus.cli token-block validate-stage5dw-handoff-continuity
    & $Python -m libreprimus.cli token-block validate-stage5dw-credential-redaction-policy
    & $Python -m libreprimus.cli token-block validate-stage5dw-governance-scope
    & $Python -m libreprimus.cli operator-console validate-source-index
    & $Python -m libreprimus.cli source-browser validate-index
    & $Python -m libreprimus.cli source-browser validate-paths
