...
- `run-consistency-checks.ps1` / `run-consistency-checks.sh`: schema, manifest, docs, registry, ignored-output, result-store, Stage 2E exploratory dry-run, Stage 2F bounded CPU execution, Stage 2G proposal approval, Stage 2H approval-gated execution, and Stage 2I approval-readiness consistency checks.
