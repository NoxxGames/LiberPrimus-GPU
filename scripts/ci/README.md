...
- `run-consistency-checks.ps1` / `run-consistency-checks.sh`: schema, manifest, docs, registry, ignored-output, result-store, Stage 2E exploratory dry-run, Stage 2F bounded CPU execution, Stage 2G proposal approval, and Stage 2H approval-gated execution consistency checks.
