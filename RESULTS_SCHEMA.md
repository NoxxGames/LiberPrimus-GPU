...
Stage 5CG adds compact post-review approval-gate integration, future operator and Deep Research decision-scaffold, Stage 5CE wording-review, and no-byte/no-execution transition-gate records only. It does not publish generated result bodies, byte streams, decoded bytes, score summaries, SQLite databases, local reports, approval decisions, or active String 4 inputs. Generated Stage 5CG diagnostics remain ignored under `experiments/results/token-block/stage5cg/`.
