...
Stage 5CI adds compact approval-record template hardening, future operator approval and Deep Research acceptance template, combined approval-gate validation preflight, activation-decision template, negative validation contract, and no-byte/no-execution transition-gate records only. It does not publish generated result bodies, byte streams, decoded bytes, score summaries, SQLite databases, local reports, approval decisions, activation decisions, or active String 4 inputs. Generated Stage 5CI diagnostics remain ignored under `experiments/results/token-block/stage5ci/`.
