...
Stage 5CM adds compact approval-record readiness-boundary records, fixture-vs-real record boundary records, end-to-end boundary validation records, future real approval-readiness preflight records, activation-decision gate-hardening records, credential-redaction policy records, and no-byte/no-execution transition-gate preservation records only. It does not publish generated result bodies, byte streams, decoded bytes, score summaries, SQLite databases, local reports, actual approval decisions, actual activation decisions, or active String 4 inputs. Generated Stage 5CM diagnostics remain ignored under `experiments/results/token-block/stage5cm/`.
