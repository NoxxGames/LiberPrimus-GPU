...
Stage 5DC adds compact Stage 5DB findings integration records, operator choice decision records, selected-option records, unselected-option preservation records, explicit-pause nonselection records, real-approval noncreation records, combined-gate and activation nonauthorization records, Stage 5CY/5DA/5BD preservation records, active-lineage preservation records, real-record boundaries, no-active/no-byte/no-execution transition-gate records, `codex-output` handoff policy records, credential-redaction preservation records, governance-scope-control records, and reviewability records only. It does not publish generated result bodies, byte streams, decoded bytes, score summaries, SQLite databases, local reports, real operator approval records, Deep Research acceptance records, combined-gate validation records, activation decisions, or active String 4 inputs. Generated Stage 5DC diagnostics remain ignored under `experiments/results/token-block/stage5dc/`.
