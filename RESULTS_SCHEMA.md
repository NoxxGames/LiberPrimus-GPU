...
Stage 5CK adds compact approval-record validation fixture packs, activation-decision fixture packs, negative validation matrix records, activation-decision review-package records, and no-byte/no-execution transition-gate preservation records only. It does not publish generated result bodies, byte streams, decoded bytes, score summaries, SQLite databases, local reports, actual approval decisions, actual activation decisions, or active String 4 inputs. Generated Stage 5CK diagnostics remain ignored under `experiments/results/token-block/stage5ck/`.
