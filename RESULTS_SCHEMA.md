...
Stage 5DA adds compact Stage 5CZ findings integration records, operator choice / pause scaffold records, choice/pause nonselection records, explicit-pause nonactivation records, Stage 5CY preservation records, Stage 5BD preservation records, active-lineage preservation records, real-record blockers, no-active/no-byte/no-execution transition-gate records, `codex-output` handoff policy records, credential-redaction preservation records, governance-scope-control records, and reviewability records only. It does not publish generated result bodies, byte streams, decoded bytes, score summaries, SQLite databases, local reports, selected operator options, selected pause records, actual operator choice/pause records, actual operator decisions, actual approval decisions, actual activation decisions, or active String 4 inputs. Generated Stage 5DA diagnostics remain ignored under `experiments/results/token-block/stage5da/`.
