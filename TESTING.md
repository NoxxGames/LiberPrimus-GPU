...
Stage 5DW extends coverage with schema validation for the high-signal number-fact review batch, review-batch selection records, Source Browser NumberFactCard overlays, overlay-only fact-card support for zero-extracted-fact entries, Source Browser loadability and path-hygiene preservation, Stage 5DV/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, 8-worker validation-cap preservation, and ignored-output checks for `experiments/results/**`, `codex-output/**`, raw source roots, and deprecated `codex_output/**`.
