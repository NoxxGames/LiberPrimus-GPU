...
Stage 5CA extends coverage with inactive sidecar review-contract schemas, exact future-runner citation validation, fail-closed trigger validation, activation-precondition validation, manifest-supersession preflight validation, no-active-ingestion/no-byte-stream guardrails, Stage 5BD run-plan preservation checks, active-lineage preservation checks, CLI validation paths, and ignored-output checks for `experiments/results/token-block/stage5ca/` and `codex-output/**`.
