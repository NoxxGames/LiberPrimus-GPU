...
Stage 5CY extends coverage with Stage 5CX warning integration, Stage 5CW real-decision preflight preservation, operator-facing option-selection preflight validation, option-selection requirements and misuse validation, Stage 5CW pytest-count reconciliation, Stage 5CU negative-fixture preservation, Stage 5CS exact six-option preservation, option-nonselection checks, real-record blocker checks, combined-gate and activation nonauthorization checks, Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5cy/`, `codex-output/**`, and deprecated `codex_output/**`.
