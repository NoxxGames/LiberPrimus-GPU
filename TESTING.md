...
Stage 5CK extends coverage with approval-record fixture-pack schemas, Stage 5CJ warning integration, Stage 5CI template preservation, Stage 5CG scaffold preservation, fixture-only operator approval and Deep Research acceptance validators, activation-decision fixture validators, negative validation matrix checks, review-package validation, combined approval-gate non-satisfaction checks, no-byte-stream transition-gate validation, no-execution transition-gate validation, Stage 5BD run-plan preservation checks, active-lineage preservation checks, CLI validation paths, and ignored-output checks for `experiments/results/token-block/stage5ck/` and `codex-output/**`.
