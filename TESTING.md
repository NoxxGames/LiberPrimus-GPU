...
Stage 5CG extends coverage with post-review approval-gate integration schemas, Stage 5CF warning integration, Stage 5CE proposal/gate preservation, operator approval and Deep Research acceptance decision-scaffold validators, combined approval-gate non-satisfaction checks, Stage 5CE wording-review validation, no-byte-stream transition-gate validation, no-execution transition-gate validation, Stage 5BD run-plan preservation checks, active-lineage preservation checks, CLI validation paths, and ignored-output checks for `experiments/results/token-block/stage5cg/` and `codex-output/**`.
