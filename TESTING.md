...
Stage 5CM extends coverage with approval-record readiness-boundary schemas, Stage 5CL warning integration, Stage 5CK fixture preservation, fixture-vs-real record boundary checks, end-to-end readiness-boundary validation, future real approval-readiness preflight validation, activation-decision gate hardening, credential-redaction/no-secret policy validation, sidecar-gate checks, Stage 5BD run-plan preservation checks, active-lineage preservation checks, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5cm/` and `codex-output/**`.
