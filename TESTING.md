...
Stage 5CW extends coverage with Stage 5CV warning integration, real-decision package preflight validation, future real-decision input requirements, preflight misuse validation, Stage 5CU negative-fixture preservation, Stage 5CS six-option preservation, option-nonselection checks, real-record blocker checks, combined-gate and activation nonauthorization checks, Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5cw/`, `codex-output/**`, and deprecated `codex_output/**`.
