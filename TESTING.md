...
Stage 5EC extends coverage with schema validation for the third Source Browser number-fact review batch, exact 20-entry selection checks, 25 review-only triangle/Page32/token-static/music/self-reference overlay checks, overlay-only fact-card rendering, Source Browser loadability checks, Stage 5EB 10-worker policy preservation, Stage 5DX/5DW/5DV/5DU/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction policy checks, and Stage 5ED routing. It runs no source-lock evidence update, source-record backfill, historical source rewrite, route extraction, byte generation, image forensics/OCR, target validation, execution, CUDA, scoring, benchmark, or solve claim.

Stage 5EB extends coverage with schema validation for validation-finalization records, Stage 5EA preservation, 10-worker local validation policy checks, full-serial-rare fallback checks, current-stage registry finalization handoff checks, generic stage-wrapper alias checks, doc-ledger tier policy checks, duration-aware pytest shard/rerun policy checks, Source Browser overlay cache reuse checks, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, and Stage 5EC routing. It runs no number-fact review batch 3, source-lock evidence update, overlay creation, route extraction, byte generation, image forensics/OCR, target validation, execution, or solve claim.

Stage 5EA extends coverage with schema validation for validation-throughput repair records, current-stage registry checks, Stage 5DZ verification, historical-test isolation checks, doc-ledger tier policy checks, validation-wrapper stage-id normalization, Source Browser number-fact overlay cache checks, pytest shard policy checks, orphan-process timeout policy checks, Stage 5DZ/5DY/5DX/5DW/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, and Stage 5EB routing. It runs no number-fact review batch 3, source-lock evidence update, overlay creation, route extraction, byte generation, image forensics/OCR, target validation, execution, or solve claim.

Stage 5DZ extends coverage with schema validation for Triangle/Page32 bounded-findings source-lock records, exact PDD153 WAY arithmetic checks, 56311 center-word52 bridge checks, ouroboric period checks, Page32 red-header 463/3299/2472 checks, 12-cell route-segment and 12+4 fold checks, negative-result review checks, overlay-only Source Browser fact-card checks, Source Browser loadability validation, ChatGPT context checks, Stage 5DY/5DX/5DW/5DV/5DU/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, and Stage 5EA routing. It runs no number-fact review batch 3, route extraction, byte generation, image forensics/OCR, target validation, execution, or solve claim.

Stage 5DY extends coverage with validation-performance and stage-isolation repair. It validates the focused/stage-fast/local-fast/full-parallel/full-serial-rare/CI profile registry, preserves the 8-worker validation cap, marks full serial pytest as rare fallback rather than default per-stage validation, hardens shared-schema collision policy, verifies validate/summary commands remain non-mutating for representative commands, records PowerShell wildcard guidance, and preserves Stage 5DX/Stage 5BD/active-lineage/no-active/no-byte/no-execution gates.

Stage 5DX extends coverage with schema validation for the second visual/red-heading/transform number-fact review batch, review-batch selection records, Source Browser NumberFactCard overlays, overlay-only fact-card support for zero-extracted-fact entries, Source Browser loadability and path-hygiene preservation, Stage 5DW/5DV/5DU/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, 8-worker validation-cap preservation, and ignored-output checks for `experiments/results/**`, `codex-output/**`, raw source roots, and deprecated `codex_output/**`.

Stage 5DV extends coverage with schema validation for Source Browser performance/path repair records, key-aware path canonicalization tests, source-root-relative image/document path resolution tests, bare filename/root-path suppression tests, duplicate present+missing basename suppression checks, path-resolution cache tests, lazy thumbnail cache tests, compact table-display tests, Source Browser `validate-paths` and `performance-smoke` commands, ChatGPT context hardening checks, Stage 5DU/5DT/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, 8-worker validation-cap preservation, and ignored-output checks for `third_party/**`, `experiments/results/**`, `codex-output/**`, and deprecated `codex_output/**`.

Stage 5DU extends coverage with schema validation for community visual/red-heading/negative-space source-lock records, six-folder file inventory checks, canonical LP page-root crosslink validation, red-runes/Gateless Gate, big-gap, star-artifact, CribbingPage15, pages 54/55, and Mobius/totient candidate checks, Operator Console number-fact overlay loading, Source Browser loadability, ChatGPT context checks, Stage 5DT/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, 8-worker validation-cap preservation, and ignored-output checks for `third_party/**`, `experiments/results/**`, `codex-output/**`, and deprecated `codex_output/**`.

Stage 5DT extends coverage with schema validation for number-fact cards, enrichment overlays, review states, review-batch plans, and Stage 5DT project-state records; deterministic NumberFactCard normalization; value-only/vague fact classification; zero-fact-not-reviewed display; overlay loading without mutating source records; GUI fact-card rendering; table/filter behavior; Source Browser loadability; Stage 5DS/5DG/5BD/active-lineage preservation; no-active/no-byte/no-execution gate checks; strict `codex-output` handoff-continuity checks; 8-worker validation-cap preservation; and ignored-output checks for `experiments/results/**`, `codex-output/**`, and deprecated `codex_output/**`.

Stage 5DS extends coverage with schema validation for expanded Music/Ouroboros/token-block static source-lock records, CicadaMusic community-theory file inventory checks, safe MP3/PDF/image metadata locks, message attachment anchors, music arithmetic candidate checks, Ouroboros/self-reference context checks, token-block primary60 static-context checks, ChatGPT context checks, Source Browser loadability checks, Stage 5DR/5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, and ignored-output/raw-source checks for `third_party/CicadaMusic/**`, `experiments/results/**`, and deprecated `codex_output/**`.

Stage 5DQ extends coverage with schema validation for Operator Console and Source Browser records, source-entry normalization, source-index determinism, manual-entry raw-blob rejection, manual override and tombstone validation, path alias resolution, column-profile loading, context-file status, CLI build/validate/summary paths, optional PySide6 smoke skip behavior, Stage 5DQ project-state guardrails, 8-worker validation-cap preservation, and ignored-output checks for `.cache/operator-console/**`, `codex-output/**`, and deprecated `codex_output/**`.

Stage 5DP extends coverage with schema validation for New Reddit Mayfly/dot/cover/ISO source-lock records, Mayfly docx/xlsx metadata, bounded workbook summaries, raw-source noncommit proofs, Mayfly/dot/front-cover/ISO/Problems candidate records, ChatGPT context-file checks, Stage 5DO preservation, Stage 5DG operator-approval preservation, Stage 5BD plan preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, CLI validation paths, 8-worker validation-cap preservation, and ignored-output checks for `experiments/results/token-block/stage5dp/`, `codex-output/**`, local `third_party/RedditStuff/**`, and deprecated `codex_output/**`.

Stage 5DO extends coverage with schema validation for Discord NumberFacts/PotentialHint source-lock records, image-anchor line-hash records, raw-source noncommit proofs, Page32 red-header arithmetic, NO-F section-flow arithmetic, doublet/word-count/GP-fact/prime-bridge/9901/pixel-colour candidates, source-browser future requirements, Stage 5DN preservation, Stage 5DG operator-approval preservation, Stage 5BD plan preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, CLI validation paths, 8-worker validation-cap preservation, and ignored-output checks for `experiments/results/token-block/stage5do/`, `codex-output/**`, local `third_party/NumberFactsCollection/**`, local `third_party/PotentialHint-3301-on-Page32/**`, and deprecated `codex_output/**`.

Stage 5DJ extends coverage with schema validation for every Stage 5DJ record, CicadaMusic source-lock validation, file hash inventory validation, safe MP3/PDF metadata validation, 761 parable metadata validation, music-number arithmetic metadata validation, music candidate-family validation, seven-option pivot non-selection checks, Stage 5DG operator-approval preservation, Stage 5BD plan preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5dj/`, `codex-output/**`, `third_party/CicadaMusic/**`, raw roots, and deprecated `codex_output/**`.

Stage 5DK extends coverage with schema validation for Fandom source-lock gap closure records, existing source-index crosswalk validation, Page 56 512-bit hash-contract validation, pivot-readiness non-selection checks, Stage 5DJ/Stage 5DG/Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5dk/`, `codex-output/**`, raw roots, and deprecated `codex_output/**`.

Stage 5DL extends coverage with schema validation for triangle/disk/quote/koan source-lock refresh records, NumberTriangleStuff validation, DiskCipherStuff validation, Reddit image source-lock validation, quote-dialogue crib validation, koan-page metadata validation, candidate-family priority validation, pivot-readiness non-selection checks, Stage 5DG/Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5dl/`, `codex-output/**`, local `third_party/**` source roots, and deprecated `codex_output/**`.

Stage 5DN extends coverage with schema validation for DiskCipher v1 source-lock registers, DiskCipherStuff file inventory, message-body and results.png source locks, Alberti/disk, p39 row-1, 56311/WYNN/WAY, Mobius-trip, doublet-suppression, RUTH/root/route/WAY, 2015 eclipse/167, pdd-153, circumference-precedent, single-I crosslink, probability-quarantine, Stage 5DM preservation, Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, governance-scope validation, CLI validation paths, 8-worker validation-cap preservation, and ignored-output checks for `experiments/results/token-block/stage5dn/`, `codex-output/**`, local `third_party/DiskCipherStuff/**` roots, and deprecated `codex_output/**`.

Stage 5DM extends coverage with schema validation for Blake visual-text, Blake web-source gap, Sacred Book overlay, Page6 magic-square precedent, full-page visual motif, Page32 Moebius/Fibonacci-prime-index, doublet-scarcity, evidence-atlas readiness, Drive/path hygiene, pivot-readiness, Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, governance-scope validation, CLI validation paths, 8-worker validation-cap preservation, and ignored-output checks for `experiments/results/token-block/stage5dm/`, `codex-output/**`, local `third_party/**` visual roots, and deprecated `codex_output/**`.

Stage 5DI extends coverage with schema validation for every Stage 5DI record, public source-lock register validation, local iddqd-v2 archive crosswalk validation, number-triangle bundle crosswalk validation, route-candidate family validation, pivot-readiness non-selection checks, dinkus visual-marker non-interpretation checks, Stage 5DG operator-approval preservation, Stage 5BD plan preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5di/`, `codex-output/**`, raw roots, local third-party source caches, and deprecated `codex_output/**`.

Stage 5DG extends coverage with Stage 5DF findings integration, real operator approval record validation, operator approval scope validation, operator approval nonactivation checks, Stage 5DE preparation preservation, Stage 5DC selected-option and choice preservation checks, five unselected-option preservation checks, Deep Research acceptance absence checks, combined-gate and activation nonauthorization checks, Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, target-context preservation, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5dg/`, `codex-output/**`, and deprecated `codex_output/**`.

Stage 5DE extends coverage with Stage 5DD warning and label-anomaly integration, real operator approval preparation-package validation, future approval-record requirement checks, Stage 5DC selected-option and choice preservation checks, five unselected-option preservation checks, real-approval noncreation checks, combined-gate and activation nonauthorization checks, Stage 5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5de/`, `codex-output/**`, and deprecated `codex_output/**`.
...
Stage 5CS extends coverage with operator-decision readiness package schemas, real-approval decision-options scaffold schemas, Stage 5CR findings integration, Stage 5CQ preservation, Stage 5CO readiness-package preservation, Stage 5CO missing-requirements preservation, Stage 5CO transition-plan preservation, option-nonselection checks, real-record blocker checks, combined-gate non-satisfaction checks, activation nonauthorization checks, Stage 5CM boundary preservation, Stage 5CK/5CI/5CG/5CE/5CC/5BD preservation, active-lineage preservation, credential-redaction preservation, handoff-continuity checks, no-active/no-byte/no-execution transition gates, CLI validation paths, and ignored-output checks for `experiments/results/token-block/stage5cs/`, `codex-output/**`, and `codex_output/**`. It confirms no operator decision, selected operator option, real approval records, Deep Research acceptance records, combined-gate satisfaction, valid activation decision, active-planning input authorization/selection, String 4 activation, byte-stream generation, execution, scoring, CUDA, benchmark, website expansion, method-status upgrade, or solve claim occurred, and Stage 5CS preserves the `8` worker cap for local validation.

Stage 5CU extends coverage with Stage 5CT findings integration, Stage 5CS decision-options preservation, decision-option negative fixtures, real-decision negative fixtures, option-selection misuse validation, option-fixture isolation policy, option-nonselection checks, real-record blocker checks, combined-gate and activation nonauthorization checks, Stage 5CS/5CQ/5CO/5CM/5CK/5CI/5CG/5CE/5CC/5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gate checks, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5cu/`, `codex-output/**`, and `codex_output/**`. It confirms all six operator-decision options remain unselected, negative fixtures cannot satisfy real decision or approval gates, no real operator decision or approval record exists, active planning input remains unauthorized and unselected, String 4 remains inactive, Stage 5BD run-plan IDs stay at `10`, active-lineage records stay at `8`, and no byte-stream generation, token execution, DWH/hash/preimage search, decode, scoring, CUDA, benchmark, website expansion, method-status upgrade, or solve claim occurred.

Stage 5CW extends coverage with Stage 5CV findings integration, review-only real-decision package preflight validation, 24 future real-decision input requirements, 24 preflight misuse cases, Stage 5CU negative-fixture preservation, Stage 5CS six-option nonselection, real-record blockers, combined-gate and activation nonauthorization checks, Stage 5BD preservation, active-lineage preservation, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5cw/`, `codex-output/**`, and `codex_output/**`. It confirms no real decision package, operator decision, approval, Deep Research acceptance, combined gate, activation decision, active input, byte stream, execution, DWH/hash/preimage search, decode, scoring, CUDA, benchmark, website expansion, method-status upgrade, or solve claim occurred.

Stage 5CY extends coverage with Stage 5CX findings integration, review-only option-selection decision preflight validation, 24 option-selection requirements, 24 option-selection misuse cases, Stage 5CW count reconciliation, Stage 5CW/5CU/5CS preservation, exact six-option nonselection, real-record blockers, combined-gate and activation nonauthorization checks, Stage 5BD preservation, active-lineage preservation, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5cy/`, `codex-output/**`, and `codex_output/**`. It confirms no option selection, real decision package, operator decision, approval, Deep Research acceptance, combined gate, activation decision, active input, byte stream, execution, DWH/hash/preimage search, decode, scoring, CUDA, benchmark, website expansion, method-status upgrade, or solve claim occurred.

Stage 5DA extends coverage with Stage 5CZ findings integration, review-only operator choice / pause scaffold validation, exact six-option preservation, choice/pause nonselection, explicit-pause nonactivation, real-record blockers, Stage 5CY preservation, Stage 5BD preservation, active-lineage preservation, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5da/`, `codex-output/**`, and `codex_output/**`. It confirms no option selection, explicit pause selection, real choice/pause record, real decision package, operator decision, approval, Deep Research acceptance, combined gate, activation decision, active input, byte stream, execution, DWH/hash/preimage search, decode, scoring, CUDA, benchmark, website expansion, method-status upgrade, or solve claim occurred.

Stage 5DE extends coverage with Stage 5DD findings and label-anomaly integration, approval-preparation package validation, 34 future approval-record requirement checks, Stage 5DC selected-option/choice preservation, five unselected-option preservation checks, real-approval noncreation, combined-gate and activation nonauthorization, Stage 5BD preservation, active-lineage preservation, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5de/`, `codex-output/**`, and `codex_output/**`. It confirms no real approval, Deep Research acceptance, gate satisfaction, activation, active input, byte stream, execution, target-class validation, Tor/network access, DWH/hash/preimage search, decode, scoring, CUDA, benchmark, website expansion, method-status upgrade, canonical-corpus activation, page-boundary finalisation, or solve claim occurred.

Stage 5DG extends coverage with Stage 5DF findings integration, real operator approval record validation, operator-approval scope and nonactivation checks, Stage 5DE preparation preservation, Stage 5DC selected-option/choice preservation, five unselected-option preservation checks, Deep Research acceptance absence, combined-gate and activation nonauthorization, Stage 5BD preservation, active-lineage preservation, target-context preservation, strict `codex-output` handoff-continuity checks, credential-redaction/no-secret policy preservation, governance-scope-control validation, CLI validation paths, 8-worker parallel-validation evidence, and ignored-output checks for `experiments/results/token-block/stage5dg/`, `codex-output/**`, and `codex_output/**`. It confirms one real operator approval record exists and is valid, while Deep Research acceptance, combined-gate satisfaction, activation, active input, byte stream, execution, target-class validation, Tor/network access, DWH/hash/preimage search, decode, scoring, CUDA, benchmark, website expansion, method-status upgrade, canonical-corpus activation, page-boundary finalisation, and solve claims remain absent.

Stage 5DJ extends coverage with CicadaMusic source-lock and music-clue pivot-integration checks. It validates 29 Stage 5DJ YAML records and schemas, confirms safe file-hash records, synthetic MP3/PDF metadata parsing, optional local 761 parable metadata, music-number metadata arithmetic, the `music_3301_instar_crab_canon_v0` family, and 7 unselected pivot options, rejects selected music pivots and audio-stego flags, rejects raw-commit flags, and confirms Stage 5DG operator approval remains only one approval component. The CLI tests cover every focused validator, `validate-stage5dj`, `stage5dj-summary`, and `build-stage5dj`; ignore-policy tests verify generated Stage 5DJ reports, `codex-output/**`, `third_party/CicadaMusic/**`, and deprecated `codex_output/**` remain out of git.

Stage 5DK extends coverage with Fandom source-lock gap closure and Page 56 hash-contract checks. It validates Stage 5DK YAML records and schemas, confirms the 14-source Fandom register, corrected older source-index crosswalks, Stage 5DI reuse links, speculative-source quarantine, Page 56 512-bit hash length/bit-count and unknown algorithm/preimage flags, all pivot options unselected, and Stage 5DJ/Stage 5DG/Stage 5BD/active-lineage preservation. The CLI tests cover every focused validator, `validate-stage5dk`, `stage5dk-summary`, and `build-stage5dk`; ignore-policy tests verify generated Stage 5DK reports, `codex-output/**`, raw roots, and deprecated `codex_output/**` remain out of git.

Stage 5DL extends coverage with triangle/disk/quote/koan source-lock refresh checks. It validates Stage 5DL YAML records and schemas, confirms local NumberTriangleStuff, DiskCipherStuff, RedditStuff, quote-dialogue crib, and koan-page metadata locks, checks 10 candidate-family updates, confirms `pdd_153_triangle_word_prime_route_v1` is future-priority metadata only, keeps the selected target null, and preserves Stage 5DG/Stage 5BD/active-lineage boundaries. The CLI tests cover every focused validator, `validate-stage5dl`, `stage5dl-summary`, and `build-stage5dl`; ignore-policy tests verify generated Stage 5DL reports, `codex-output/**`, local third-party source roots, and deprecated `codex_output/**` remain out of git.

Stage 5DM extends coverage with visual-route source-lock addendum checks. It validates Stage 5DM YAML records and schemas, confirms Blake/Sacred Book/Page6/Page32/full-page/doublet/evidence-atlas/path-hygiene addendum records, verifies the manually supplied Page32 prime-index relation, keeps Page32 mod-153 projections future-only, keeps the selected target null, and preserves Stage 5DL plus Stage 5DG/Stage 5BD/active-lineage boundaries. The CLI tests cover every focused validator, `validate-stage5dm`, `stage5dm-summary`, and `build-stage5dm`; ignore-policy tests verify generated Stage 5DM reports, `codex-output/**`, local third-party visual source roots, and deprecated `codex_output/**` remain out of git.

Stage 5DN extends coverage with DiskCipher v1 source-lock and triangle/circumference bridge checks. It validates Stage 5DN YAML records and schemas, confirms message-body and results.png source locks, checks p39 row-1, 56311/WYNN/WAY, doublet-suppression, probability-quarantine, circumference-precedent, and pdd-triangle update records, keeps target selection and execution disabled, and preserves Stage 5DM plus Stage 5DG/Stage 5BD/active-lineage boundaries. The CLI tests cover every focused validator, `validate-stage5dn`, `stage5dn-summary`, and `build-stage5dn`; ignore-policy tests verify generated Stage 5DN reports, `codex-output/**`, local DiskCipher source roots, and deprecated `codex_output/**` remain out of git.

Stage 5DO extends coverage with Discord NumberFacts and pixel-colour source-lock addendum checks. It validates Stage 5DO YAML records and schemas, confirms NumberFactsCollection and PotentialHint inventories, message source locks, image-anchor hashes, Page32 arithmetic, NO-F section-flow, doublet/word-count/GP-fact/pixel-colour candidates, source-browser future requirements, target non-selection, and Stage 5DN plus Stage 5DG/Stage 5BD/active-lineage boundaries. The CLI tests cover every focused validator, `validate-stage5do`, `stage5do-summary`, and `build-stage5do`; ignore-policy tests verify generated Stage 5DO reports, `codex-output/**`, local NumberFacts/PotentialHint source roots, and deprecated `codex_output/**` remain out of git.

Stage 5DP extends coverage with New Reddit Mayfly / dot / cover / ISO source-lock addendum checks. It validates Stage 5DP YAML records and schemas, confirms RedditStuff required-folder representation, Mayfly docx/xlsx locks, bounded workbook grid summaries, candidate-only records, ChatGPT context-file updates, target non-selection, Stage 5DO preservation, and Stage 5DG/Stage 5BD/active-lineage boundaries. The CLI tests cover every focused validator, `validate-stage5dp`, `stage5dp-summary`, and `build-stage5dp`; ignore-policy tests verify generated Stage 5DP reports, `codex-output/**`, local RedditStuff source roots, and deprecated `codex_output/**` remain out of git.

Stage 5DQ extends coverage with Operator Console Source Browser checks. It validates Operator Console schemas, Source Browser entry normalization, index building from committed metadata, manual entry creation and blob-size rejection, override/tombstone target validation, path aliases, column profiles, context-file helpers, CLI build/validate/summary commands, `source-browser` aliases, Stage 5DQ project-state guardrails, and optional Qt import behavior when PySide6 is present. Ignore-policy tests verify `.cache/operator-console/**`, `codex-output/**`, and deprecated `codex_output/**` remain out of git.

Stage 5DS extends coverage with expanded Music/Ouroboros/token-block static source-lock schemas, ignored community-theory file inventory checks, safe MP3/PDF/image metadata locks, message attachment anchors, review-only candidate records, Source Browser loadability, ChatGPT context update, Stage 5DR/Stage 5DG/Stage 5BD/active-lineage preservation, no-active/no-byte/no-execution gates, strict `codex-output` handoff checks, and raw/generated ignore-policy tests for `third_party/CicadaMusic/**`, `experiments/results/**`, and deprecated `codex_output/**`.

Stage 5DT extends coverage with Operator Console number-fact card reviewability checks. It validates Stage 5DT YAML records and schemas, confirms normalized NumberFactCard output, flags value-only facts as enrichment-needed, keeps zero extracted facts distinguishable from reviewed-no-facts states, applies overlay records without mutating source locks, renders GUI cards when PySide6 is present, adds Source Browser filters for number-fact review states, preserves Stage 5DS plus Stage 5DG/Stage 5BD/active-lineage boundaries, and verifies no fact backfill, no source-lock rewrite, no review-batch execution, no active input, no byte streams, no execution, no CUDA, and no solve claim. Ignore-policy tests verify `experiments/results/**`, `codex-output/**`, and deprecated `codex_output/**` remain out of git.

Stage 5DU extends coverage with community visual/red-heading/negative-space source-lock checks. It validates Stage 5DU YAML records and schemas, confirms six ignored local thread folders are represented as metadata only, checks canonical page-image root crosslinks, verifies each focused candidate family remains review-only and unselected, confirms NumberFactCard enrichment overlays load without rewriting source records, preserves Stage 5DT plus Stage 5DG/Stage 5BD/active-lineage boundaries, and verifies no review-batch execution, no community code execution, no OCR/image forensics, no route extraction, no target selection, no active input, no byte streams, no execution, no CUDA, and no solve claim. Ignore-policy tests verify `third_party/**`, `experiments/results/**`, `codex-output/**`, and deprecated `codex_output/**` remain out of git.

Stage 5DV extends coverage with Source Browser performance and path-canonicalization repair checks. It validates key-aware/source-root-aware path collection, bare-filename suppression, duplicate present/missing path suppression, path and thumbnail cache behavior, lazy raw-preview behavior, Stage 5DU/5DT/5DG/5BD/active-lineage preservation, and no-active/no-byte/no-execution gates while keeping the planned Stage 5DW number-fact review batch unperformed.

Stage 5DX extends coverage with source-lock number-fact review batch 002. It validates Stage 5DX YAML records and schemas, confirms exactly 20 selected visual/red-heading/transform bridge records, 23 review-only NumberFactCard overlays, overlay-only support preservation, Source Browser loadability, Stage 5DW/5DV/5DU/5DG/5BD/active-lineage preservation, closed no-active/no-byte/no-execution gates, strict `codex-output` handoff continuity, credential-redaction preservation, and governance-scope false matrices. Ignore-policy tests verify generated reports, raw sources, third-party roots, `codex-output/**`, and deprecated `codex_output/**` remain out of git.

Stage 5DY adds focused test files under `tests/python/test_stage5dy_*.py` and staged script profiles under `scripts/ci/run-stage-validation.*`. Use focused tests during implementation, `stage-fast` or `local-fast` before commit, and `full-parallel` near final when broad validation is needed. Full serial `python -m pytest -q tests/python` is a rare fallback for suspected parallel races or CI-only reproduction, not the default local validation loop. In PowerShell, expand pytest file lists with `Get-ChildItem` or concrete paths rather than assuming Bash-style wildcard expansion.

Stage 5DZ adds focused test files under `tests/python/test_stage5dz_*.py` and extends staged validation profiles to accept `-Stage stage5dz`. The tests cover Triangle/Page32 bounded-findings schemas, exact PDD153 WAY arithmetic, Page32 route-segment/fold metadata, overlay-only Source Browser cards, Source Browser loadability, ChatGPT context updates, Stage 5DY/5DX/5DW/5DV/5DU and Stage 5DG/5BD/active-lineage preservation, no-active/no-byte/no-execution gates, and Stage 5EA routing. Stage 5DZ validation does not run number-fact review batch 3, route extraction, route stream generation, byte-stream generation, image forensics/OCR, execution, or solve claims.

Stage 5EB adds focused test files under `tests/python/test_stage5eb_*.py` and makes staged validation wrappers default to `stage5eb` with 10-worker local defaults/caps. The tests cover Stage 5EB schemas, Stage 5EA preservation, 10-worker policy, serial-pytest rare fallback policy, current-stage registry finalization policy, generic stage aliases, doc-tier policy, duration-aware shard/rerun metadata, Source Browser cache reuse, no-active/no-byte/no-execution gates, strict `codex-output` handoff continuity, and ignored-output policy. Stage 5EB validation does not run number-fact review batch 3, source-lock evidence updates, overlay creation, route extraction, route stream generation, byte-stream generation, image forensics/OCR, execution, or solve claims.

Stage 5EC adds focused test files under `tests/python/test_stage5ec_*.py` for the third Source Browser number-fact review batch. The tests cover Stage 5EC schemas, exact 20-entry selection, 25 review-only overlays, overlay-only fact cards, Source Browser loadability, Stage 5EB/5DX/5DW/5DV/5DU/5DG/5BD preservation, active-lineage preservation, no-active/no-byte/no-execution gates, strict `codex-output` handoff continuity, credential-redaction policy, and ignored-output policy. Stage 5EC validation does not update source-lock evidence, rewrite historical records, backfill facts directly to source records, select targets, run routes, generate byte streams, execute OCR/image/audio/stego/native/VM/CUDA/scoring/benchmark work, or make solve claims.

Stage 5ED adds focused test files under `tests/python/test_stage5ed_*.py` for the fourth Source Browser number-fact review batch. The tests cover Stage 5ED schemas, exact 20-entry selection, 25 review-only overlays, overlay-only fact cards, Source Browser loadability, Stage 5EC and Stage 5EB preservation, historical preservation, active-lineage preservation, no-active/no-byte/no-execution gates, strict `codex-output` handoff continuity, credential-redaction policy, and ignored-output policy. Stage 5ED validation does not update source-lock evidence, rewrite historical records, backfill facts directly to source records, select targets, run routes, generate byte streams, execute OCR/image/audio/stego/native/VM/CUDA/scoring/benchmark work, or make solve claims.
