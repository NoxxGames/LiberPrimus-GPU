...
- For Stage 5AT/5AU-style token case-review pack work, require committed `data/token-block/stage5at-*`, `data/token-block/stage5au-*`, `data/project-state/stage5at-*`, and `data/project-state/stage5au-*` metadata records, active ambiguity classes matching committed data, grouped challenge records, canonical-transcription challenge records, blank human decision templates, generated ignored review-pack files under `human-review-packs/stage5at/token-case-review/` or `human-review-packs/stage5au/token-case-review-v2/`, generated ignored `experiments/results/token-block/stage5at/` and `experiments/results/token-block/stage5au/` reports, no canonical transcription changes, no OCR/AI/ML/LLM vision token reading, no semantic image interpretation, no hidden-content image forensics, no stego execution, no hash/preimage search, no decode attempt, no CUDA, no benchmarks, no scored experiments, no page-boundary finalisation, and no solve claim.
...
- For Stage 5BD-style token-block preflight dry-run work, require Stage 5BB/5AZ/5AY/5AW inputs, deterministic run-plan IDs, future result-path validation, fixture-only records, Stage 5BB validation-evidence consolidation, archive marker policy/scripts, generated ignored `experiments/results/token-block/stage5bd/` reports, ignored `deep-research-repo-zips/stage5bd/` markers/ZIPs, `codex-output/**` ignored, no real byte-stream generation, no variant materialisation, no Cartesian enumeration, no DWH/hash/preimage search, no decode, no scoring, no OCR/AI/ML/LLM vision, no stego, no CUDA, no benchmarks, no website expansion, no method-status upgrade, and no solve claim.
- For Stage 5BF-style historical-route source-lock work, require local-only `third_party/CicadaSolversIddqd` metadata handling, archive tree digest records, compact inventory/trust/taxonomy records, ignored `experiments/results/historical-route/stage5bf/` reports, ignored `deep-research-content-packs/stage5bf/` and `deep-research-repo-zips/stage5bf/` outputs, `codex-output/**` ignored, no online clone, no live scraping, no online PGP verification, no stego tools, no token-block byte streams, no DWH/hash/preimage search, no decode, no scoring, no OCR/AI/ML/LLM vision, no CUDA, no benchmarks, no website expansion, no method-status upgrade, and no solve claim.
