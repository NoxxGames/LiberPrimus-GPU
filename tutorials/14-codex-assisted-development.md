...
- For Stage 5AF-style source-harvester work, require source-manifest records, collection priorities, clue-target categories, local-only tool policy, dry-run planning, research-bundle scaffolds, generated ignored reports, `codex-output/**` ignored, no Google Drive storage, no live broad scraping, no raw download commits, no raw archive processing, no CUDA execution, no CUDA source changes, no new kernels, no benchmarks, no scored experiments, no website expansion, and no solve claim.
...
For Stage 5AF and later source-harvester work, keep raw outputs under ignored local roots such as `source-harvester-output/`, `harvest-output/`, or `research-inputs/`. Google/Dropbox/Colab sources require manual export to local ignored storage; do not use Google Drive as a project storage location.
