...
- For Stage 5AT-style token case-review pack work, require committed `data/token-block/stage5at-*` and `data/project-state/stage5at-*` metadata records, active ambiguity classes matching committed data, grouped challenge records, canonical-transcription challenge records, human decision templates, generated ignored `human-review-packs/stage5at/token-case-review/` review-pack files, generated ignored `experiments/results/token-block/stage5at/` reports, no canonical transcription changes, no OCR/AI/ML/LLM vision token reading, no semantic image interpretation, no hidden-content image forensics, no stego execution, no hash/preimage search, no decode attempt, no CUDA, no benchmarks, no scored experiments, no page-boundary finalisation, and no solve claim.
