...
When using Codex after Stage 5J, cite the Stage 5J Gematria CUDA kernel records, Stage 5I Gematria CUDA preparation records, Stage 5H Gematria mod-29 `shift_score` contract records, Stage 5G parity-reporting/device-code audit records, Stage 5F synthetic `shift_score_kernel` parity records, Stage 5E selected contract, Stage 4O CPU batch parity expectations, Stage 4P unified result surfaces, Stage 4Q benchmark plans, Stage 5A target-plan/scaffold/gate records, Stage 5B harness/backend/matrix records, Stage 5C build/device records, and Stage 5D native CPU backend/threading records. Do not let Gematria CUDA parity reporting turn into broad CUDA implementation, real Liber Primus CUDA execution, or GPU benchmarking without an explicit prompt.
