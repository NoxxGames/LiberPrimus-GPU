...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5I Gematria mod-29 `shift_score` synthetic CUDA parity preparation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Existing CUDA code is scaffold, smoke-test infrastructure, the Stage 5F synthetic `shift_score_kernel`, Stage 5G reporting/device-code hardening, Stage 5H metadata/fixture contract records, and Stage 5I preparation metadata only.

### Deferred future work

- Stage 5J Gematria mod-29 shift_score synthetic CUDA parity kernel implementation after Stage 5I preparation.
...
- Latest CUDA preparation stage: Stage 5I wrote `1` kernel-preparation record, `1` CUDA-C ABI plan record, `1` validation-vector record, and `1` implementation-checklist record for `gematria_mod29_shift_score_kernel`. It added no CUDA source, ran no CUDA, used no real Liber Primus CUDA data, and made no solve claim.
...
- Next: Stage 5J Gematria mod-29 shift_score synthetic CUDA parity kernel implementation.
