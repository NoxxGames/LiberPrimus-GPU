...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AD-fix bounded p56 CUDA parity mismatch investigation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- CPU batch transform API: Stage 4H provides deterministic CPU-only input stream, transform candidate, result, summary, adapter coverage, and parity-contract records. Later stages extend this into result-store, benchmark-planning, CUDA-contract, native-conformance, prime-minus-one, synthetic CUDA parity, Stage 5AC compact reporting/bounded-p56 preflight metadata, the Stage 5AD bounded p56 CUDA mismatch record, and the Stage 5AD-fix reference-contract investigation. Stage 5AB adds a document staleness gate over those operational claims so current/next-stage text, website deferral, and CUDA-boundary wording cannot silently drift. This is infrastructure for future native/CUDA parity, not a broad experiment runner.
...
- Stage 5AE - corrected bounded p56 CUDA formula parity reporting and reference-contract repair.
...
- Latest prime-minus-one CUDA reporting stage: Stage 5AD-fix writes bounded p56 mismatch hash-lineage, token/stream/formula trace, hash-material, reference-contract, root-cause, repair-readiness, guardrail, next-stage decision, and summary records. It proves the Stage 5AD CUDA/formula hash matches the Stage 5X formula hash and not the Stage 5L candidate-major expected hash, preserves Stage 5AD as failed, modifies no CUDA source, adds no kernels, runs no CUDA/full-p56/unsolved-page work, benchmarks nothing, processes no raw data, and makes no solve claim.
- Next: Stage 5AE - corrected bounded p56 CUDA formula parity reporting and reference-contract repair.
