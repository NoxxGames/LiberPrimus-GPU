...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5Z prime-minus-one CUDA contract preparation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5AA - prime-minus-one CUDA synthetic kernel implementation and parity.
...
- Latest prime-minus-one CUDA contract stage: Stage 5Z wrote `2` contract records, `1` kernel ABI record, `1` host-runner contract record, `11` buffer contract records, `7` validation-vector records, `4` future parity-plan records, `2` result-store compatibility records, `1` full-p56 blocker record, `6` scored-experiment deferral records, `1` implementation-readiness gate record, and `7` next-stage decision records. It did not run native parity or CUDA, modify CUDA source, add kernels, benchmark, publish generated bodies, or make solve claims.
- Next: Stage 5AA - prime-minus-one CUDA synthetic kernel implementation and parity.
