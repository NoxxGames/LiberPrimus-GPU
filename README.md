...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AB markdown staleness detection hardening and stale-doc repair. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- CPU batch transform API: Stage 4H provides deterministic CPU-only input stream, transform candidate, result, summary, adapter coverage, and parity-contract records. Later stages extend this into result-store, benchmark-planning, CUDA-contract, native-conformance, prime-minus-one, and synthetic CUDA parity metadata through Stage 5AA. Stage 5AB adds a document staleness gate over those operational claims so current/next-stage text, website deferral, and CUDA-boundary wording cannot silently drift. This is infrastructure for future native/CUDA parity, not a broad experiment runner.
...
- Latest doc-quality stage: Stage 5AB adds dynamic stage parsing, operational Markdown staleness scanning, source-of-truth records, an operational file map, CI integration, and stale-doc repairs without CUDA execution, CUDA source changes, new kernels, benchmarks, scored experiments, raw-data processing, website expansion, generated-output publication, or solve claims.
- Next: Stage 5AC - selected from Stage 5AA outcome after stale-doc repair.
