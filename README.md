...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5BO token-case human-review errata integration and String 4 full-branch reclassification. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest historical-route/token-block crosswalk stage: Stage 5BO integrates the operator-corrected ignored decision-template as compact errata metadata without committing template bodies. It records `8` changed token-case records, confirms case `199` changes from `0I|0j|OI|Oj` to `0I|0l|OI|Ol`, confirms case `198` changes from `1I|1j` to `1i|1j`, reclassifies String 4 as `full_branch_match` for inactive planning with `249` canonical matches, `6` Stage 5AW-supported noncanonical positions, `1` operator-errata-supported noncanonical position, and `0` unsupported positions, while leaving canonical transcription, active token-block manifests, Stage 5AW/5AY/5AZ records, and Stage 5BD dry-run gates unchanged.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5BO updates current/next-stage records to Stage 5BP.
- Next: Stage 5BP - Deep Research review of Stage 5BO operator-errata integration before dry-run ingestion.
