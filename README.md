...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DG real operator approval record metadata. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest historical-route/token-block approval-readiness stage: Stage 5DG consumes Stage 5DF assistant/operator review context as compact metadata, preserves Stage 5DE preparation and Stage 5DC selected option `prepare_real_operator_approval_record`, creates one valid real operator approval record, marks only the operator-approval component satisfied, preserves 8 active-lineage paths and 10 Stage 5BD run-plan IDs, caps local validation at 8 workers, records explicit pause unselected, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, and future token-block execution blocked.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial. Stage 5DG preserves the local parallel-validation and pytest cap at 8 workers for Stage 5CM and later.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DG updates current/next-stage records to Stage 5DG complete and Stage 5DH next.
- Next: Stage 5DH - Review of Stage 5DG real operator approval record, without activation.
