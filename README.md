...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5V native Candidate Batch ABI conformance fixtures. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest native Candidate Batch ABI conformance stage: Stage 5V wrote `2` native adapter records, `7` conformance fixtures, executed `3` raw-data-free Python reference fixtures, kept `4` fixtures shape-only, wrote `7` token-buffer records, `2` schedule records, `7` score-vector records, `1` top-k record, `3` result-store conformance records, `8` implementation-status records, and `9` next-stage decision records. It left the C++ adapter deferred, ran no CUDA or native/CUDA CMake, added no kernels, benchmarked nothing, and selected Stage 5W prime-minus-one stream native parity contract preparation.
- Next: Stage 5W - prime-minus-one stream native parity contract preparation.
