...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DW source-lock number-fact review batch 001. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5DW number-fact review batch 001 records are reviewability overlays only. They add high-signal NumberFactCard overlays and overlay-only card support, but do not rewrite historical source locks, backfill facts directly, select targets, authorize byte streams or execution, or make solve evidence.
...
- Stage 5DW high-signal source-lock number-fact review batch 001 with 20 selected evidence/candidate records reviewed, 37 review-only NumberFactCard overlays added, overlay-only fact cards enabled for zero-extracted-fact entries, 1529 Source Browser entries validated, zero spurious root image/document paths, zero duplicate present+missing path pairs, Stage 5DV/5DU/5DT/5DG/5BD preserved, no direct source-record fact backfill, no source-lock rewrite, no target selected, no byte streams, no execution, and no solve claim.
...
- Stage 5DW: Source-lock number-fact review batch 001 complete with `20` selected records reviewed, `37` review-only NumberFactCard overlays, overlay-only fact-card support, `1529` Source Browser entries validated, `1528` committed records scanned, `53` fact cards after overlays, `0` spurious root image/document paths, `0` duplicate present+missing path pairs, Stage 5DV/Stage 5DU/Stage 5DT/Stage 5DG/Stage 5BD/active-lineage preservation, no direct historical fact backfill, no source-lock rewrite, no target selected, no byte streams, no execution, and no solve claim.
...
- Latest operator-console/source-lock stage: Stage 5DW implements source-lock number-fact review batch 001 as reviewability overlay metadata. It validates `1529` Source Browser entries, adds `37` overlays for `20` selected records, supports overlay-only fact cards, suppresses spurious root paths and duplicate present+missing path pairs to `0`, preserves Stage 5DV/Stage 5DU and Stage 5BD/active-lineage boundaries, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, DWH/hash/preimage search, OCR/image forensics/AI interpretation, CUDA, community-code execution, raw-source mutation, source-lock rewrite, direct historical fact backfill, and future token-block execution blocked.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DW updates current/next-stage records to Stage 5DW complete and Stage 5DX next.
- Next: Stage 5DX - Operator/assistant source-lock number-fact review batch 2, without execution.
