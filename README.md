...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5BD token-block preflight dry-run implementation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest token-block stage: Stage 5BD implements a no-byte-stream dry-run planning layer. It records run-plan IDs `10`, dry-run family combinations `11520`, unique variant families `10`, taxonomy memberships `11`, null-control families `6`, alphabet controls `6`, reading-order controls `8`, page-split controls `4`, source controls `3`, active Stage 5AW and Stage 5AZ locks `true`, Stage 5AV and Stage 5AY active inputs `false`, future result paths validated `true`, future result paths written `false`, fixture-only records `true`, Stage 5BB validation placeholders consolidated `true`, archive marker policy `true`, execution gates blocked `true`, real byte streams `false`, variant outputs `false`, DWH/hash search `false`, decode `false`, scoring `false`, CUDA `false`, and solve claim `false`.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5BD updates current/next-stage records to Stage 5BE.
- Next: Stage 5BE - Deep Research review of token-block preflight dry-run implementation, archive/evidence hygiene, and execution-gate enforcement.
