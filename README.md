...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AZ bounded preflight manifest integrity repair. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest token-block stage: Stage 5AZ repairs the Stage 5AY bounded preflight manifest integrity gap. It records duplicate family ID count before/after repair `1/0`, repaired unique variant families `10`, taxonomy memberships `11`, branch budget changed `false`, Stage 5AW repaired branch manifest used `true`, Stage 5AV branch manifest used `false`, execution gates `8`, manifest-integrity gate status `design_satisfied_execution_still_blocked`, and Deep Research readiness `true`. It supersedes the Stage 5AY bounded variant-family manifest for review and runs no byte-stream generation, token experiment, decode, hash/preimage search, OCR/AI/ML, LLM/vision token reading, semantic image interpretation, hidden-content forensics, stego, CUDA, benchmark, scored experiment, or solve claim.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AZ updates current/next-stage records to Stage 5BA.
- Next: Stage 5BA - Deep Research review of repaired bounded token-block preflight manifest and execution gates.
