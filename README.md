...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5Q controlled solved-fixture-safe Gematria shift_score expansion candidate mapping. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest Gematria CUDA expansion-candidate stage: Stage 5Q wrote `10` candidate inventory records, `3` new mapped direct-translation candidates, `5` consumed Stage 5L/5M/5O control records, `2` blocked original-family records, `3` native parity hashes, `3` result-store preflight records, and `1` Stage 5R-ready gate. It publishes no generated CUDA result bodies, upgrades no method family to solved, runs no CUDA, modifies no CUDA source, adds `0` kernels, and makes no benchmark, speedup, unsolved-page CUDA, real Liber Primus CUDA-data, website-expansion, canonical-corpus, page-boundary, or solve claim.
- Next: Stage 5R controlled expanded solved-fixture-safe Gematria shift_score CUDA parity run.
