...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5P controlled solved-fixture CUDA result-store integration. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest Gematria CUDA result-store stage: Stage 5P wrote `5` compact result-store integration records, `5` Stage 4I-compatible score-summary records, `7` method-status impact records, `4` generated-body policy records, `6` controlled expansion candidate records, and `1` summary record. It integrates only Stage 5O repeat parity metadata, publishes no generated CUDA result bodies, upgrades no method family to solved, runs no CUDA, modifies no CUDA source, adds `0` kernels, and makes no benchmark, speedup, unsolved-page CUDA, real Liber Primus CUDA-data, website-expansion, canonical-corpus, page-boundary, or solve claim.
- Next: Stage 5Q controlled solved-fixture-safe Gematria shift_score expansion candidate mapping.
