...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DU community visual/red-heading/negative-space source-lock metadata. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest operator-console/source-lock stage: Stage 5DU source-locks community visual/red-heading/negative-space material as compact review-only metadata. It represented `6` ignored local thread folders, inventoried `234` files, crosslinked `75` canonical LP page images, created `72` candidate records, created or enriched `12` number-fact cards/overlays, loaded `1490` Source Browser entries, preserves Stage 5DT and Stage 5BD/active-lineage boundaries, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, DWH/hash/preimage search, OCR/image forensics/AI interpretation, CUDA, community-code execution, raw-source mutation, and future token-block execution blocked.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DU updates current/next-stage records to Stage 5DU complete and Stage 5DV next.
- Next: Stage 5DV - Operator/assistant source-lock number-fact review batch 1, without execution.
