...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AA prime-minus-one CUDA synthetic kernel implementation and parity. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- CPU batch transform API: Stage 4H provides deterministic CPU-only input stream, transform candidate, result, summary, adapter coverage, and parity-contract records. Stage 4O expands this with solved-fixture-safe streams, adapter coverage records, deterministic parity expectations, and score-summary compatibility checks. Stage 4P makes those result and score surfaces comparable with older result-store summaries. Stage 4Q adds benchmark and future parity planning gates. Stage 5A adds CUDA target plans, non-targets, parity scaffolds, and implementation gates. Stage 5B adds CUDA parity harness plans, parity fixtures, backend capability profiles, and future-kernel matrix rows. Stage 5C adds no-GPU-safe CUDA build profiles, toolchain detection, device detection, and optional smoke-build readiness metadata. Stage 5D adds native C++ CPU backend, deterministic threading, and native/Python parity records. Stage 5E selects `shift_score_kernel` as the first future CUDA kernel contract and maps it to the native synthetic shift parity adapter. Stage 5F implements only that synthetic-only CUDA parity kernel and records matching synthetic hash metadata. Stage 5G reports the Stage 5F hash match and hardens `.cu`/`.cuh` CUDA-facing code to a conservative C-style subset. Stage 5H defines the separate numeric Gematria mod-29 contract and native fixture while keeping solved-fixture CUDA execution blocked. Stage 5I adds Gematria CUDA preparation records for a CUDA-C ABI and validation vectors. Stage 5J adds only the synthetic numeric `gematria_mod29_shift_score_kernel` and matching parity records. Stage 5K reports that parity, audits device-code subset compliance, and records solved-fixture-safe blockers without CUDA execution. Stage 5L maps committed solved-fixture-safe streams into exact Gematria token buffers and native output-token hashes. Stage 5M runs the existing kernel over exactly those five mapped buffers and matches all native hashes. Stage 5N reports the matches and gates future scope. Stage 5O repeats the exact five-buffer pack and prepares compact result-store/score-summary preflight. Stage 5P integrates those hashes into Stage 4P/Stage 4I-compatible compact records without publishing generated bodies. Stage 5Q maps three additional direct-translation solved-fixture-safe candidates and prepares native parity hashes without running CUDA. Stage 5R runs the existing kernel over exactly those three mapped candidates and matches all Stage 5Q native hashes. Stage 5S integrates those three expanded parity matches into compact result-store and score-summary records. Stage 5T classifies solved-family CUDA readiness and records candidate batch ABI gaps. Stage 5U defines Candidate Batch ABI v0 token, key, stream, score-vector, top-k, backend, and result-store contracts. Stage 5V proves no-GPU Candidate Batch ABI conformance with a pure Python reference adapter, raw-data-free fixtures, deterministic output hashes, score-vector/top-k shape records, and compact result-store compatibility records. Stage 5W prepares the source-backed prime-minus-one stream native parity contract, including p56 bounded readiness and full-token-buffer blockers. Stage 5X executes only the Stage 5W ready synthetic and bounded p56 no-GPU Python-reference mappings, matches both expected hashes, and keeps full p56 blocked pending a full committed token buffer. Stage 5Y integrates compact reporting/readiness metadata. Stage 5Z prepares the prime-minus-one CUDA contract, and Stage 5AA implements and runs only the synthetic `prime_minus_one_stream_kernel_v0` vector with a matching Stage 5Z hash while keeping p56/full-p56, unsolved pages, scored experiments, benchmarks, generated-body publication, website expansion, and solve claims blocked. This is infrastructure for future native/CUDA parity, not a broad experiment runner.
...
- Existing CUDA code is scaffold, smoke-test infrastructure, the Stage 5F synthetic uppercase-Latin `shift_score_kernel`, Stage 5G reporting/device-code hardening, Stage 5H metadata/fixture contract records, Stage 5I preparation metadata, and the Stage 5J synthetic numeric `gematria_mod29_shift_score_kernel` only. Stage 5K adds reporting/preflight records only; Stage 5L adds token-mapping/native-fixture records; Stage 5M adds host-runner-only plumbing plus exact solved-fixture parity records and adds no new kernel.

### Deferred future work

- Stage 5AB - prime-minus-one CUDA synthetic parity reporting and bounded-p56 CUDA parity preflight.
...
- Latest prime-minus-one CUDA synthetic stage: Stage 5AA wrote `1` kernel implementation record, `1` CUDA run record, `1` parity record, `1` device-subset audit record, `2` result-store preflight records, `2` p56/full-p56 blocker records, `6` scored-experiment deferral records, and `4` next-stage decision records. It ran only the synthetic validation vector, matched the Stage 5Z expected hash, and did not run p56/full-p56, unsolved pages, scored experiments, benchmarks, generated-body publication, website expansion, or solve claims.
- Next: Stage 5AB - prime-minus-one CUDA synthetic parity reporting and bounded-p56 CUDA parity preflight.
