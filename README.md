...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5EA validation throughput and historical-test isolation repair. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5EA updates current/next-stage records to Stage 5EA complete and Stage 5EB next.
- Latest operator-console/source-lock addendum: Stage 5DZ source-locks assistant/operator bounded findings for PDD153 triangle and Page32. Stage 5DZ does not select a target, execute routes or produce route streams, generate byte streams, perform image forensics/OCR, or accept a solve claim. Stage 5EA verifies those records while deferring number-fact review batch 3 to Stage 5EB.
- Next: Stage 5EB - Operator/assistant source-lock number-fact review batch 3, without execution.
