...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AX parallel validation harness and fast CI check orchestration. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest token-block stage: Stage 5AW repairs Stage 5AV possible-token parser contamination, finding `3` malformed prose-as-token extras, preserving `10` valid reviewer-extra tokens, preserving `2` visual placeholders as unmappable review-only options, recalculating primary-60 mappable/unmappable options to `99/65`, rebuilding the compact branch manifest with branch upper-bound log10 `24.434582`, and keeping canonical transcription changed `false`, automatic case resolution `false`, variant byte streams generated `false`, and full branch enumeration `false`. It runs no decode, hash/preimage search, LP-page OutGuess, Deep Research, OCR/AI/ML, LLM/vision token reading, semantic image interpretation, hidden-content forensics, CUDA, benchmark, scored experiment, or solve claim.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, 16-worker local sharded pytest fallback, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AX updates current/next-stage records to Stage 5AY.
- Next: Stage 5AY - bounded token-block preflight manifest design without execution.
