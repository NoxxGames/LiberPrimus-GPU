...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5BI Fandom source-lock triage and original-archive crosswalk integration. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest historical-route stage: Stage 5BI records Fandom page triage and original-archive crosswalk metadata without execution. It records Fandom page triage records `30`, item source-lock candidates `18`, original archive crosswalk candidates `12`, verified archive-equivalent crosswalks `7`, probable archive path candidates `3`, source gaps `5`, negative controls `4`, local archive present `true`, spreadsheet present `true`, canonical transcription changed `false`, active token-block manifests changed `false`, and Stage 5BJ original-archive crosswalk closure as next.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5BI updates current/next-stage records to Stage 5BJ.
- Next: Stage 5BJ - Original-archive crosswalk closure for high-priority Fandom-derived candidates, without execution.
