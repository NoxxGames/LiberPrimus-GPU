...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AR page 49-51 original-image pixel-coordinate and page-split source-gap closure. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest token-block stage: Stage 5AR records `3` selected original page images for pages 49-51, `15` image variants, source-backed row split `10/13/9`, `256` original-image pixel-coordinate records, validation `valid_with_review_required`, `9` unresolved case-ambiguity classes, and source-lock/null-control/DWH coordinate updates. It runs no decode, hash/preimage search, LP-page OutGuess, Deep Research, OCR/AI/ML, semantic image interpretation, hidden-content forensics, CUDA, benchmark, scored experiment, or solve claim.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AR updates current/next-stage records to Stage 5AS.
- Next: Stage 5AS - Deep Research page 49-51 original-image coordinate review and bounded token-block preflight planning.
