...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AT token case-policy human review pack and canonical-transcription challenge set. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5AU - manual human review of token case challenge pack.
...
- Stage 5AT token case-policy human review pack and canonical-transcription challenge set.
...
- Stage 5AT: token case human-review pack complete.
...
- Latest token-block stage: Stage 5AT records `9` active ambiguity classes, `203` grouped case-review challenges, `212` canonical-transcription challenge records, `1015` generated local crops, `9` review sheets, variant-classifier repair status `true`, doc-drift repair status `true`, canonical transcription changed `false`, and human review decisions present/integrated `false/false`. It runs no decode, hash/preimage search, LP-page OutGuess, Deep Research, OCR/AI/ML, LLM/vision token reading, semantic image interpretation, hidden-content forensics, CUDA, benchmark, scored experiment, or solve claim.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AT updates current/next-stage records to Stage 5AU.
- Next: Stage 5AU - manual human review of token case challenge pack.
