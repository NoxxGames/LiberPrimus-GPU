...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5K Gematria `shift_score` CUDA parity reporting and solved-fixture-safe preflight. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest Gematria CUDA parity reporting stage: Stage 5K converts the Stage 5J hash match into durable parity/preflight records, confirms device-code subset compliance, and keeps solved-fixture CUDA execution blocked pending token mapping, score-summary parity, no-unsolved guardrails, and future-stage approval.
- Durable staged plan: [`docs/roadmap/staged-plan.md`](docs/roadmap/staged-plan.md).
- Next: Stage 5L solved-fixture-safe Gematria shift_score token mapping and native parity fixture preparation.
