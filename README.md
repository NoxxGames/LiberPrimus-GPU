...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AV token case-review decision integration and compact variant-branch manifest. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5AW - bounded token-block preflight manifest design without execution.
...
- Latest token-block stage: Stage 5AV integrates the filled local Stage 5AU decision template with `203` validated decisions, `126` keep-current confirmations, `77` unresolved variant branches, `13` reviewer-extra possible tokens, primary-60 mappable/unmappable options `98/65`, branch upper-bound log10 `24.258491`, compact branch manifest `true`, canonical transcription changed `false`, automatic case resolution `false`, variant byte streams generated `false`, and full branch enumeration `false`. It runs no decode, hash/preimage search, LP-page OutGuess, Deep Research, OCR/AI/ML, LLM/vision token reading, semantic image interpretation, hidden-content forensics, CUDA, benchmark, scored experiment, or solve claim.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AV updates current/next-stage records to Stage 5AW.
- Next: Stage 5AW - bounded token-block preflight manifest design without execution.
