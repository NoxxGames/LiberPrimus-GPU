...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AK community-facts observations integration and number-fact claim curation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5AK community-facts observations integration and number-fact claim curation.
...
- Latest source-harvester stage: Stage 5AK integrates `11` ignored community-facts files, `10` ordered attachment records, `12` claim records, `4` correction records, `15` clue-category records, `16` arithmetic-preflight checks, `1` source-card update, and `5` content-index updates. It preserves `10` bundles ready for private Deep Research, keeps public website-ready bundles at `0`, commits only compact metadata, and performs no network fetch, online clone, Google Drive storage, raw source commit, Deep Research execution, hypothesis generation/execution, OCR, AI/ML interpretation, image/stego/audio execution, CUDA, benchmark, scored experiment, website expansion, or solve claim.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AK preserves that gate and updates current/next-stage records to Stage 5AL.
- Next: Stage 5AL - Deep Research source inventory and reliability prompt.
