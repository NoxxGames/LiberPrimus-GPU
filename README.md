...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AW decision possible-token parser cleanup and branch-manifest repair. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AW updates current/next-stage records to Stage 5AX.
- Next: Stage 5AX - bounded token-block preflight manifest design without execution.
