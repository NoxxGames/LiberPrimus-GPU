...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DI recent clue source-lock and pivot-readiness metadata. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5DJ - Target-priority decision package, without execution.
...
- Stage 5DI recent clue source-lock and pivot-readiness package with Stage 5DG operator approval preserved, 17 compact source-lock records, 8 candidate families, 6 unselected pivot options, Deep Research acceptance absent, combined gate unsatisfied, activation/active input/byte streams/target validation/Tor/network/execution blocked, and Stage 5DJ selected as the next review/decision package.
...
- Stage 5DI: recent clue source-lock and pivot-readiness package complete with 6 public web source locks, 11 local archive source-lock records, 8 clue/candidate families, 7 route-candidate families, 1 visual-marker candidate, 6 unselected pivot options, Stage 5DG operator approval preserved, Stage 5BD run-plan IDs preserved at 10, active lineage preserved at 8, Deep Research acceptance absent, combined gate unsatisfied, activation unauthorized, byte streams blocked, execution blocked, target validation absent, Tor/network access absent, and no solve claim.
...
- Latest historical-route/token-block approval-readiness stage: Stage 5DI source-locks recent clue material as compact metadata, crosswalks iddqd-v2 and number-triangle local archive sources, records 8 candidate families and 6 unselected pivot options, preserves the Stage 5DG operator-approval component, preserves 8 active-lineage paths and 10 Stage 5BD run-plan IDs, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, and future token-block execution blocked.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial. Stage 5DI preserves the local parallel-validation and pytest cap at 8 workers for Stage 5CM and later.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DI updates current/next-stage records to Stage 5DI complete and Stage 5DJ next.
- Next: Stage 5DJ - Target-priority decision package, without execution.
