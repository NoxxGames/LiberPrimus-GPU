...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DP New Reddit Mayfly / dot / cover / ISO source-lock metadata. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5DQ - Lightweight source-lock browser GUI implementation, without puzzle execution.
...
- Latest historical-route/token-block approval-readiness stage: Stage 5DP adds New Reddit Mayfly / dot / cover / ISO source-lock metadata, records Mayfly docx/xlsx locks and bounded workbook facts, creates 23 review-only candidate records, preserves the Stage 5DG operator-approval component, preserves 8 active-lineage paths and 10 Stage 5BD run-plan IDs, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, DWH/hash/preimage search, OCR/image forensics/AI interpretation, CUDA, GUI implementation, and future token-block execution blocked.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial. Stage 5DP preserves the local parallel-validation and pytest cap at 8 workers for Stage 5CM and later.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DP updates current/next-stage records to Stage 5DP complete and Stage 5DQ next.
- Next: Stage 5DQ - Lightweight source-lock browser GUI implementation, without puzzle execution.
