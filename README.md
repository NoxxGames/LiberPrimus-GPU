...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DK Fandom source-lock gap closure and Page 56 hash-contract refinement metadata. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5DL - Target-priority decision package, without execution.
...
- Latest historical-route/token-block approval-readiness stage: Stage 5DK source-locks 14 Fandom pages as compact metadata or reviewable gaps, corrects older malformed source-index URLs, quarantines the speculative harmonic-key source, preserves the Page 56 512-bit hash target with unknown algorithm/preimage, preserves the Stage 5DG operator-approval component, preserves 8 active-lineage paths and 10 Stage 5BD run-plan IDs, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, DWH/hash/preimage search, and future token-block execution blocked.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial. Stage 5DK preserves the local parallel-validation and pytest cap at 8 workers for Stage 5CM and later.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DK updates current/next-stage records to Stage 5DK complete and Stage 5DL next.
- Next: Stage 5DL - Target-priority decision package, without execution.
