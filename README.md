...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DT Operator Console number-fact card and reviewability infrastructure. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest operator-console stage: Stage 5DT adds number-fact card reviewability to the Liber Primus Operator Console Source Browser as review-only GUI/CLI infrastructure. It loaded `1387` source-browser entries from committed metadata, extracted `20` number-fact cards, marked `13` vague value-only facts for enrichment, marked `1383` zero-fact entries as not reviewed, planned `7` bounded review batches, preserves Stage 5DS and Stage 5BD/active-lineage boundaries, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, DWH/hash/preimage search, OCR/image forensics/AI interpretation, CUDA, raw-source mutation, and future token-block execution blocked.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DT updates current/next-stage records to Stage 5DT complete and Stage 5DU next.
- Next: Stage 5DU - Operator/assistant source-lock number-fact review batch 1, without execution.
