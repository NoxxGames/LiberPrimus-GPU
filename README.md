...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5BJ original-archive crosswalk closure for high-priority Fandom-derived candidates. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest historical-route stage: Stage 5BJ closes Stage 5BI original/archive crosswalk gaps without execution. It records Stage 5BI crosswalk candidates consumed `12`, Stage 5BJ closure rows `12`, exact 2014 surface targets `3`, exact 512-hex surfaces locked `3`, surface source files found `3`, page-body crosswalk rows `7`, boards thread found `true`, media-equivalence rows `8`, source gaps closed/carried/new `4` / `3` / `2`, local archive present `true`, source snapshots present `true`, spreadsheet present `true`, canonical transcription changed `false`, active token-block manifests changed `false`, and Stage 5BK planning constraint integration as next.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5BJ updates current/next-stage records to Stage 5BK.
- Next: Stage 5BK - Historical-route planning constraint integration, without execution.
