...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5DL triangle / disk / quote / koan source-lock refresh metadata. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest historical-route/token-block approval-readiness stage: Stage 5DL source-locks the triangle/disk/Reddit/quote/koan evidence cluster as compact metadata, records `pdd_153_triangle_word_prime_route_v1` as a future operator preference only, preserves the Stage 5DG operator-approval component, preserves 8 active-lineage paths and 10 Stage 5BD run-plan IDs, caps local validation at 8 workers, and leaves Deep Research acceptance, combined-gate satisfaction, activation authorization, active input, dry-run ingestion, byte-stream generation, target-class validation, Tor access, route extraction, DWH/hash/preimage search, and future token-block execution blocked.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial. Stage 5DL preserves the local parallel-validation and pytest cap at 8 workers for Stage 5CM and later.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5DL updates current/next-stage records to Stage 5DL complete and Stage 5DM next.
- Next: Stage 5DM - Target-priority decision package, without execution.
