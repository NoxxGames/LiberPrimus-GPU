...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AU token case-review pack usability fix and glyph-tight crop rebuild. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5AV - manual human review of token case challenge pack v2.
...
- Latest token-block stage: Stage 5AU records Stage 5AT review-pack usability `false`, rebuilds a v2 pack with all `203` case-review challenges and all `212` canonical-transcription challenges visible, generates `2436` local crop/overlay files, records glyph-candidate crop count `203`, context crop count `609`, row-context crop count `203`, overlay count `609`, crop-quality statuses `usable_with_context:123` and `good_for_review:80`, canonical transcription changed `false`, automatic case resolution `false`, and human review decisions present/integrated `false/false`. It runs no decode, hash/preimage search, LP-page OutGuess, Deep Research, OCR/AI/ML, LLM/vision token reading, semantic image interpretation, hidden-content forensics, CUDA, benchmark, scored experiment, or solve claim.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AU updates current/next-stage records to Stage 5AV.
- Next: Stage 5AV - manual human review of token case challenge pack v2.
