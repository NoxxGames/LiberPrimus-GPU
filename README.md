...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5BK historical-route planning constraint integration and iddqd-v2 source-lock addendum. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5BL - Deep Research review of historical-route planning constraints and iddqd-v2 source-lock integration, without execution.
...
- Latest historical-route stage: Stage 5BK integrates Stage 5BF/5BI/5BJ historical-route context without execution. It records iddqd-v2 selected path `third_party/CiadaSolversIddqd_v2`, file count `309`, byte strings `4`, exact 512-hex strings `4`, String 1-3 crosswalks `3`, String 4 page49-51 crosswalk created `true`, transcription locks `2`, translation/key-lineage records `4`, positive-control context records `11`, historical family planning statuses `9`, source-gap severity records `7`, Stage 5BJ errata warnings `1`, canonical transcription changed `false`, active token-block manifests changed `false`, and Stage 5BL Deep Research review as next.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5BK updates current/next-stage records to Stage 5BL.
- Next: Stage 5BL - Deep Research review of historical-route planning constraints and iddqd-v2 source-lock integration, without execution.
