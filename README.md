...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5L solved-fixture-safe Gematria `shift_score` token mapping and native parity fixture preparation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest Gematria CUDA reporting stage: Stage 5K wrote `1` parity report, `1` device-code audit, `5` solved-fixture-safe preflight records, `1` score-summary preflight record, and `1` summary record. It reports the same hash match, records `7` blockers with `needs_token_mapping: 5`, adds no kernels, changes no CUDA source, executes no CUDA, and keeps solved-fixture CUDA blocked.
- Latest solved-fixture mapping stage: Stage 5L wrote `5` token-mapping records, `5` native parity records, `1` output-hash contract, `1` score-summary shape, and `1` summary record. It maps all `5` candidate streams, prepares `5` native output-token hashes, reduces blockers from `7` to `1`, and keeps solved-fixture CUDA blocked pending explicit future-stage approval.
...
- Latest Gematria solved-fixture mapping stage: Stage 5L converts Stage 5K's non-execution blockers into source-backed Gematria token buffers, native output-token hashes, output-hash contracts, and Stage 4I-compatible score-summary shape records while keeping CUDA execution blocked pending explicit future-stage approval.
- Durable staged plan: [`docs/roadmap/staged-plan.md`](docs/roadmap/staged-plan.md).
- Next: Stage 5M first solved-fixture-safe Gematria shift_score CUDA parity run, pending explicit future-stage approval.
