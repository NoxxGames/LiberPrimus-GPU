...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5W prime-minus-one stream native parity contract preparation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Next: Stage 5X - prime-minus-one stream no-GPU native parity execution and result-store preflight.
