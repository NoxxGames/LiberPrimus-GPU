...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5AM static research website renderer and webserver export package. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest source-harvester/website stage: Stage 5AM renders Stage 5AL records into an ignored uploadable static metadata index at `website-export/stage5am/research-index/` with `9` pages, `8` data JSON files, `61` source-card records, `58` content records, `12` community-claim records, `10` bundles, `7` publication gates, and `26` missing-source records. It keeps public website-ready at `0`, commits only renderer metadata and contracts, and performs no network fetch, online clone, Google Drive storage, raw source commit, public website publication, Deep Research execution, hypothesis generation/execution, OCR, AI/ML interpretation, image/stego/audio execution, CUDA, benchmark, scored experiment, or solve claim.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5AM updates current/next-stage records to Stage 5AN.
- Next: Stage 5AN - Deep Research source inventory and reliability prompt.
