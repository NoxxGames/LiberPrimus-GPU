...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5BN String 4 unsupported-position source-gap closure and human-review pack preparation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest historical-route/token-block crosswalk stage: Stage 5BN audits the single Stage 5BM unsupported String 4 position without execution. It records target token index `199`, Stage 5AP canonical token `0I`, String 4 inferred token `0l`, Stage 5AW active support for `0l` `false`, local spreadsheet target-row support for `0l` `true`, inactive addendum status `proposed_inactive_review_only`, canonical transcription changed `false`, active token-block manifests changed `false`, and Stage 5BO inactive addendum integration as next.
...
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5BN updates current/next-stage records to Stage 5BO.
- Next: Stage 5BO - String 4 inactive token-option addendum integration, without execution.
