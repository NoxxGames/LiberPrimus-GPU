...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5F first synthetic-only CUDA parity kernel implementation. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5G shift_score CUDA parity reporting and solved-fixture-safe adapter preflight after the Stage 5F synthetic-only kernel implementation.
...
- Latest CUDA kernel stage: Stage 5F implemented `shift_score_kernel` for the Stage 5D synthetic uppercase-shift fixture, recorded matching native/CUDA synthetic hash metadata, and kept real Liber Primus CUDA data use, GPU benchmarks, speedup claims, broad execution, raw-data processing, website expansion, and solve claims out of scope.
- Durable staged plan: [`docs/roadmap/staged-plan.md`](docs/roadmap/staged-plan.md).
- Next: Stage 5G shift_score CUDA parity reporting and solved-fixture-safe adapter preflight.
