...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5CY operator real-decision package readiness review integration and option-selection decision preflight. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Stage 5CZ - Deep Research review of Stage 5CY option-selection decision preflight and validation-count reconciliation, without execution.
...
- Latest historical-route/token-block approval-readiness stage: Stage 5CY consumes Stage 5CX findings as compact metadata, preserves Stage 5CW real-decision package preflight as review-only, preserves Stage 5CU negative fixtures and Stage 5CS readiness/options records, reconciles the Stage 5CW pytest-count warning, preserves 8 active-lineage paths and 10 Stage 5BD run-plan IDs, caps local validation at 8 workers, and leaves String 4 active input, dry-run ingestion, byte-stream generation, active-planning authorization, approval-gate satisfaction, activation authorization, and future token-block execution blocked.
- Latest validation stage: Stage 5AX adds opt-in parallel validation infrastructure with `10` parallel-safe commands, `6` serial commands, `1` blocked command, separated logs, failure aggregation, and safety audits while keeping git/GitHub/network/generated-output-writing operations serial. Stage 5CY preserves the local parallel-validation and pytest cap at 8 workers for Stage 5CM and later.
- Latest doc-staleness stage: Stage 5AH repairs README stage-ledger coverage, operational-file-map coverage, and current/next-stage consistency checks before curated extraction. Stage 5CY updates current/next-stage records to Stage 5CY complete and Stage 5CZ next.
- Next: Stage 5CZ - Deep Research review of Stage 5CY option-selection decision preflight and validation-count reconciliation, without execution.
