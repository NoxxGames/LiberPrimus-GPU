...
These are not permanent project exclusions unless marked as safety rules. They describe the current implementation boundary after Stage 5R controlled expanded solved-fixture-safe Gematria shift_score CUDA parity. Future experiments must stay bounded, reviewable, and reproducible before larger campaigns begin. CUDA and broad campaigns are deferred, not permanently excluded.
...
- Latest Gematria expanded solved-fixture CUDA parity stage: Stage 5R wrote `3` run records, attempted/passed/failed/skipped CUDA `3/3/0/0`, wrote `3` parity records, `1` boundary record, `3` result-store preflight records, and `3` score-summary preflight records. It uses only the existing `gematria_mod29_shift_score_kernel` over the three Stage 5Q mapped direct-translation candidates, adds `0` kernels, changes no device arithmetic, publishes no generated CUDA result bodies, upgrades no method family to solved, and makes no benchmark, speedup, unsolved-page CUDA, real Liber Primus CUDA-data, website-expansion, canonical-corpus, page-boundary, or solve claim.
- Next: Stage 5S expanded solved-fixture Gematria CUDA parity reporting and result-store integration.
