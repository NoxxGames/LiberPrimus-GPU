...
Stage 5AF Cicada source harvester and archive/visual/numeric provenance inventory tooling is complete. It adds local-only source manifests, clue categories, dry-run planning, research-bundle scaffolds, hash/inventory/extraction tooling, and strict raw-output guardrails without live broad scraping or raw-data publication.

The next recommended prompt is Stage 5AG - run source harvester on user-provided downloads and build initial source-lock inventory. Website expansion is deferred to a future unnumbered project. Broad CUDA implementation remains deferred except for explicit, synthetic-only or solved-fixture-safe parity stages until CPU batch, scorer, review workflow, source-lock
