...
Stage 5DC is not an experiment. It is metadata-only operator choice selection; generated diagnostics under `experiments/results/token-block/stage5dc/` remain ignored. It consumes Stage 5DB findings, preserves Stage 5DA, Stage 5CY, Stage 5CW, Stage 5CU, Stage 5CS, Stage 5BD run-plan IDs, and active lineage, records selected option `prepare_real_operator_approval_record`, preserves the other five options unselected, records no explicit pause selected, strict `codex-output` completion-summary handoff discipline, and the 8-worker local parallel-validation cap. It does not create a real operator approval record, create a real decision package, create a Deep Research acceptance record, satisfy the gate, authorize or select active planning input, activate String 4, supersede manifests, mutate Stage 5BD plans, generate byte streams, materialise variants, execute token-block work, search DWH/hash targets, decode, score, run CUDA, benchmark, expand the website, upgrade method status, activate the canonical corpus, finalise page boundaries, or make solve claims.
