...
Stage 5CG is not an experiment. It is metadata-only post-review approval-gate integration and active-planning-input decision-record scaffolding; generated diagnostics under `experiments/results/token-block/stage5cg/` remain ignored. It preserves Stage 5CE and Stage 5CC contracts, creates future operator and Deep Research decision scaffolds without satisfying either approval, closes no-byte-stream and no-execution transition gates, preserves Stage 5BD plans and active lineage, but it does not activate String 4, supersede manifests, mutate Stage 5BD plans, generate byte streams, materialise variants, execute token-block work, search DWH/hash targets, decode, score, run CUDA, benchmark, expand the website, upgrade method status, or make solve claims.
