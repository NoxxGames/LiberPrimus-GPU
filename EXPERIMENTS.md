...
Stage 5DA is not an experiment. It is metadata-only operator choice / pause decision scaffolding; generated diagnostics under `experiments/results/token-block/stage5da/` remain ignored. It preserves Stage 5CY option-selection preflight, Stage 5CW real-decision preflight, Stage 5CU negative-fixture records, Stage 5CS readiness/options records, Stage 5BD run-plan IDs, and active lineage, records no option selected, no explicit pause selected, strict `codex-output` completion-summary handoff discipline, and the 8-worker local parallel-validation cap. It does not create a real choice/pause record, create a real decision package, create an operator decision, select an option, select explicit pause, create approval records, satisfy the gate, authorize or select active planning input, activate String 4, supersede manifests, mutate Stage 5BD plans, generate byte streams, materialise variants, execute token-block work, search DWH/hash targets, decode, score, run CUDA, benchmark, expand the website, upgrade method status, or make solve claims.
