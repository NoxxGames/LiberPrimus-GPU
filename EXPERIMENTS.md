...
Stage 5CU is not an experiment. It is metadata-only operator-decision option negative-fixture hardening; generated diagnostics under `experiments/results/token-block/stage5cu/` remain ignored. It preserves Stage 5CS readiness/options records, Stage 5CQ operator-decision scaffold records, Stage 5CO readiness and transition records, Stage 5CM boundary hardening, Stage 5CK fixture-only records, Stage 5CI templates, Stage 5CG scaffolds, Stage 5CE proposal-package records, Stage 5CC contracts, Stage 5BD run-plan IDs, and active lineage, records 41 decision-option negative fixtures, 10 real-decision negative fixture classes, 13 option-selection misuse cases, strict `codex-output` completion-summary handoff discipline, and the 8-worker local parallel-validation cap. It does not create an operator decision, select an option, create approval records, satisfy the gate, authorize or select active planning input, activate String 4, supersede manifests, mutate Stage 5BD plans, generate byte streams, materialise variants, execute token-block work, search DWH/hash targets, decode, score, run CUDA, benchmark, expand the website, upgrade method status, or make solve claims.
