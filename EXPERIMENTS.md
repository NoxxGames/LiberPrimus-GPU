...
Stage 5CK is not an experiment. It is metadata-only approval-record validation fixture packaging and activation-decision review-package scaffolding; generated diagnostics under `experiments/results/token-block/stage5ck/` remain ignored. It preserves Stage 5CI, Stage 5CG, Stage 5CE, Stage 5CC, and Stage 5BD records, creates fixture-only operator approval and Deep Research acceptance validators, creates activation-decision fixture validators, records a negative validation matrix, and keeps no-byte-stream and no-execution transition gates closed. It does not create approval records, satisfy the gate, authorize or select active planning input, activate String 4, supersede manifests, mutate Stage 5BD plans, generate byte streams, materialise variants, execute token-block work, search DWH/hash targets, decode, score, run CUDA, benchmark, expand the website, upgrade method status, or make solve claims.
