...
Stage 5DU is not an experiment. It is a metadata-only community visual/red-heading/negative-space source-lock addendum; generated diagnostics and the local Codex handoff under `codex-output/stage5du-codex-completion.md` remain ignored. It preserves Stage 5DT, Stage 5DG operator approval, Stage 5BD run-plan IDs, active lineage, and the no-active/no-byte/no-execution gates while source-locking six ignored local community-thread folders, recording review-only visual candidate families, adding Operator Console number-fact enrichment overlays, and routing Stage 5DV source-lock number-fact review batch 1. It does not create Deep Research acceptance, satisfy the combined gate, select a target, authorize activation or active input, generate byte streams, run route extraction, run OCR/image forensics/AI interpretation, execute community code, execute token-block/native/VM/CUDA work, validate target classes, use Tor/network access, search DWH/hash targets, decode, score, benchmark, expand the website, upgrade method status, activate the canonical corpus, finalise page boundaries, or make solve claims.
