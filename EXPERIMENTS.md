...
Stage 5CI is not an experiment. It is metadata-only approval-record template hardening and activation-decision validation preflight; generated diagnostics under `experiments/results/token-block/stage5ci/` remain ignored. It preserves Stage 5CG, Stage 5CE, Stage 5CC, and Stage 5BD records, hardens future operator approval and Deep Research acceptance templates, hardens the combined approval-gate validator, hardens the activation-decision template, and keeps no-byte-stream and no-execution transition gates closed. It does not create approval records, satisfy the gate, authorize or select active planning input, activate String 4, supersede manifests, mutate Stage 5BD plans, generate byte streams, materialise variants, execute token-block work, search DWH/hash targets, decode, score, run CUDA, benchmark, expand the website, upgrade method status, or make solve claims.
