...
Stage 5AW repairs the Stage 5AV possible-token parser before preflight design proceeds. Stage 5AX then adds validation infrastructure only. Stage 5AY planning uses `data/token-block/stage5aw-repaired-token-variant-branch-manifest.yaml`, not the original Stage 5AV branch manifest. Stage 5AZ repairs the duplicate bounded variant-family ID before review, Stage 5BB makes the Stage 5AW branch manifest active through the no-execution runner registry, and Stage 5BD locks that lineage for dry-run planning. Neither stage changes Stage 5AV decisions or Stage 5AW branch semantics. Valid reviewer extras are preserved, visual placeholders are review-only and primary-60 unmappable, and prose fragments are stored only in malformed-fragment audit records.
## Stage 5BD Status

Stage 5BD keeps Stage 5AV decision records historical and consumes Stage 5AW repaired branch metadata through the active-manifest lock.
