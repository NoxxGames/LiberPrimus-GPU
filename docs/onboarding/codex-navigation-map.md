...
For Stage 5DE real operator approval preparation work, start with `docs/onboarding/stage5de-real-operator-approval-preparation-workflow.md` and run `token-block build-stage5de`, the focused Stage 5DE findings/label-anomaly/preparation/requirements/preservation/noncreation/gate/handoff/governance validators, `validate-stage5de`, and `stage5de-summary`. Treat the preparation package as metadata only; it does not create a real operator approval record or authorize approval, activation, active input, target validation, Tor access, byte streams, or execution.

For Stage 5DG real operator approval record work, start with `docs/onboarding/stage5dg-real-operator-approval-record-workflow.md` and run `token-block build-stage5dg`, the focused Stage 5DG findings/approval/scope/nonactivation/preservation/gate/target/handoff/governance validators, `validate-stage5dg`, and `stage5dg-summary`. Treat the real operator approval record as one approval component only; it does not create Deep Research acceptance, satisfy the combined gate, authorize activation, select active input, validate targets, use Tor/network access, generate byte streams, or execute token-block work.

For Stage 5DI recent clue source-lock and pivot-readiness work, start with `docs/experiments/stage-5di-recent-clue-source-lock.md` and run `token-block build-stage5di`, the focused Stage 5DI source-lock/crosswalk/candidate/pivot/dinkus/preservation/gate/handoff/credential/governance validators, `validate-stage5di`, and `stage5di-summary`. Treat the package as source-lock metadata only; it preserves Stage 5DG operator approval, selects no pivot target, creates no Deep Research acceptance, keeps the combined gate unsatisfied, and authorizes no byte streams, route extraction, target validation, Tor/network access, execution, CUDA, or solve claim.
