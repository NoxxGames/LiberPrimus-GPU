...
Stage 5BD consumes the Stage 5AW repaired branch manifest through the Stage 5BB active-manifest registry and Stage 5BD active-manifest lock. The original Stage 5AV branch manifest remains inactive as an active runner input; use it only for explicit historical diagnostics.
...
Stage 5BD keeps Stage 5AW repaired parser output as the active branch source and keeps Stage 5AV inactive for active loads.
