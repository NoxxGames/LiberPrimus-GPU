...
2. Inspect the committed Stage 5AP records under `data/token-block/` and `data/stego/`, then inspect the Stage 5AR coordinate records under `data/token-block/stage5ar-*`, the Stage 5AT review-pack records under `data/token-block/stage5at-*`, and the Stage 5AU review-pack v2 records under `data/token-block/stage5au-*`.
