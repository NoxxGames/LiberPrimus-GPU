...
Use this workflow when running the Stage 5AF harvester locally. For Stage 5AG local source inventory over `third_party/`, use `docs/onboarding/local-source-inventory-workflow.md` after validating the manifest. Stage 5AH repaired the documentation gate; Stage 5AI completed curated extraction metadata; Stage 5AJ integrated UsefulFilesAndIdeas metadata and extraction/redaction/scraper policy while keeping raw/extracted bodies ignored.
