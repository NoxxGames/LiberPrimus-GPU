...
- For Stage 3S-style execution, state the exact manifest ID, candidate cap, generated output paths, and that no other post-Discord manifest may run in the same stage.
...
worktrees. For Stage 3R/3S-style work, also keep generated Discord review bundles, topic shards, promotion audit JSONL, post-Discord candidate JSONL, summary JSON, and root report copies out of staging unless copied into `docs/` intentionally.
