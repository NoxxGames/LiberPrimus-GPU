...
Stage 5D adds a native C++ CPU backend baseline beside the Python CPU batch reference. Stage 5E
selects `shift_score_kernel` as the first future CUDA kernel contract and maps it back to the
native synthetic shift adapter. Python still
...
Python reference, Stage 5D native output hashes, and Stage 5E contract records before any kernel
