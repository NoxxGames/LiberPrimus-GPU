...
The mapping records are not broad CUDA execution permission. They prove that the future parity harness has
...
parity can run. Stage 5M consumed that approval for the exact five mapped buffers, and Stage 5N
records that future expansion still requires a controlled gate.
