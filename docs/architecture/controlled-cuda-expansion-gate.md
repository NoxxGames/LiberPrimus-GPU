...
- `exact_repeat_verification_gate`: consumed by Stage 5O for an exact repeat of the same five Stage 5L buffers.
...
- `result_store_score_summary_gate`: Stage 5O result-store and score-summary preflight is complete; compact integration remains a Stage 5P task.
...
The gate is not execution permission except where a later stage explicitly scopes that execution. Stage 5O did not approve broad solved-fixture CUDA or unsolved-page CUDA.
