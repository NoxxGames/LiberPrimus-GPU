...
Likely manual fix: create an initial Wiki page in the GitHub UI so GitHub initializes the Wiki git
repository, then rerun:

```powershell
.\scripts\github\sync-tutorials-to-wiki.ps1 --Publish --Repo NoxxGames/LiberPrimus-GPU
```

Do not create or commit a fake `.wiki-worktree/`; the publish script should use the real Wiki remote
after GitHub initializes it.
